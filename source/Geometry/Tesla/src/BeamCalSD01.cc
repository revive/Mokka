...
BeamCalSD01::BeamCalSD01(G4String name, 
			 G4double Rinner,
			 G4double Router,
			 G4double xAngle,
			 G4double zStart,
...
//  G4cout << "SJA:TAG: BeamCalSD01 Constructor:" 
//	 << " Rinner = " << Rinner 	
//	 << " Router = " << Router 	
//	 << " xAngle = " << xAngle 	
//	 << " zStart = " << zStart 	
...
//	   << G4endl;  
  ...
  //AS: zMin should reference the starting point?
//   const G4double zMin = zStart-;//envVolumeLength/2.0 - nLayers * dLayer ;
  const G4double zMin = envVolumeCenterZ-envVolumeLength/2.;
  ...
  bcalParam->setDoubleVal("cylinder_starting_phi", sPhi ) ;//phi cylinder starTing phi??? does this hurt anybody???
...
BeamCalSD01::~BeamCalSD01(void)
{
	delete theEncoder;
}

void BeamCalSD01::Initialize(G4HCofThisEvent *)
{
...
G4bool BeamCalSD01::ProcessHits(G4Step *aStep, G4TouchableHistory *)
{
  ...
  //  G4cout << "Volume Name of Hit " << volumeName << G4endl;
  ...
  //  BeamCalSD00 was LrNum =
  //  aStep->GetPreStepPoint()->GetPhysicalVolume()->GetCopyNo(); But this gets
  //  the SensorLayerSol I guess, which is always the same, since we are going
  //  to use onle BeamCalLayer and place it several times.

  //Get The Mother of the Volume!, doesn't work so easily with GetMotherLogical
  //LrNum = aStep->GetPreStepPoint()->GetPhysicalVolume()->GetCopyNo();
  G4TouchableHandle theTouchable = aStep->GetPreStepPoint()->GetTouchableHandle();
  G4int depth = theTouchable->GetHistoryDepth();
  LrNum = theTouchable->GetVolume(depth-2)->GetCopyNo();
  side =  theTouchable->GetVolume(depth-1)->GetCopyNo();

  // G4cout << "History Depth " << depth << G4endl;
  // G4cout << "Depth   Name " << theTouchable->GetVolume(depth)->GetName() << G4endl; 
  // G4cout << "Depth-1 Name " << theTouchable->GetVolume(depth-1)->GetName() << "   "
  //  	 << "CopyNumber " << theTouchable->GetVolume(depth-1)->GetCopyNo() << G4endl; 
  // G4cout << "Depth-2 Name " << theTouchable->GetVolume(depth-2)->GetName() << "   "
  //  	 << "Depth-2 CopyNumber " << theTouchable->GetVolume(depth-2)->GetCopyNo() << G4endl; 
  // // G4cout << "Depth-3 Name " << theTouchable->GetVolume(depth-3)->GetName() << G4endl; 
...
//	 << G4endl; 
  ...
  // G4cout << "LayerNumber " << LrNum << G4endl;
  //side is taken from CaloVolume CopyNumber
  if ( side == 1 ) {
    hitPos.rotateY(-xAngle/2.*mrad);
  } else if ( side == 2 ) {
    hitPos.rotateY(xAngle/2.*mrad);
  } else {
    G4cout << "Something is terribly wrong with the BeamCal!" << G4endl;
    exit(120);
  }
...
//  }
  ...
  G4double z = zStart + envVolumeLength - nLayers * dLayer + dAbsorber + dSensor/2.0 + (LrNum -1) * dLayer;   
  ...
  if(abs(LrNum)==nLayers+1) {
    ...
    // G4cout << "LRNum  " << LrNum << G4endl;
    // G4cout << "Side " << side << G4endl;
    z = zStart + dSensor/2.0 ;
    //    G4cout << "z PairMon  " << z << G4endl;
  } else {
    //    G4cout << "z BC  " << z << G4endl;
  }
...
//  G4ThreeVector *localCellPos = new G4ThreeVector(r*cos(phi),r*sin(phi),z);
  ...
  // // //This gives us the SensorLayerSol
  // // G4TouchableHandle theTouchable = aStep->GetPreStepPoint()->GetTouchableHandle();
  // // G4int depth = theTouchable->GetHistoryDepth();
  // G4ThreeVector &transl = (G4ThreeVector&)theTouchable->GetTranslation(depth-1);
  // G4RotationMatrix *rotat; 
  // if(LrNum == nLayers+1) {
  //   transl = (G4ThreeVector&)theTouchable->GetTranslation(depth-1);
  //   rotat  = (G4RotationMatrix*)theTouchable->GetRotation(depth-1);
  // } else {
  //  transl = (G4ThreeVector&)theTouchable->GetTranslation(depth-3);
  //  rotat  = (G4RotationMatrix*)theTouchable->GetRotation(depth-3);
  // }

  // G4AffineTransform aftrans(rotat, transl);

  G4ThreeVector GlobalCellPos(r*cos(phi),r*sin(phi),z);// = aftrans.Inverse().TransformPoint(*localCellPos);

  if ( side == 1 ) {
    GlobalCellPos.rotateY(xAngle/2.*mrad);
  } else if ( side == 2 ) {
    GlobalCellPos.rotateY(-xAngle/2.*mrad);
  } else {
    G4cout << "Something is terribly wrong with the BeamCal!" << G4endl;
...
//    G4cout << "SJA:TAG:BeamCalSD01: Current Volume Name: " << currentVolumeName << G4endl;
//    G4cout << "SJA:TAG:BeamCalSD01: Next Volume Name: " << nextVolumeName << G4endl;
//    G4cout << "SJA:TAG:BeamCalSD01: Limiting Process: " << limitingProcessName << G4endl;
//    G4cout << "SJA:TAG:BeamCalSD01: GlobalCellPos Volume Name: " << myVolumeName << G4endl;
//    G4cout << "SJA:TAG:BeamCalSD01: GlobalCellPos X: " << GlobalCellPos(0) << G4endl;
//    G4cout << "SJA:TAG:BeamCalSD01: GlobalCellPos Y: " << GlobalCellPos(1) << G4endl;
//    G4cout << "SJA:TAG:BeamCalSD01: GlobalCellPos Z: " << GlobalCellPos(2) << G4endl;
//    G4cout << "SJA:TAG:BeamCalSD01: localCellPos X: " << localCellPos->x() << G4endl;
//    G4cout << "SJA:TAG:BeamCalSD01: localCellPos Y: " << localCellPos->y() << G4endl;
//    G4cout << "SJA:TAG:BeamCalSD01: localCellPos Z: " << localCellPos->z() << G4endl;
//    exit(99);
//  }
//
//  delete myNavigator;
...
void BeamCalSD01::EndOfEvent(G4HCofThisEvent *HCE)
{
  if (HCID < 0)
    {
...
void BeamCalSD01::LoadEvent(FILE *eventFile)
{
