...
// *********************************************************
//
// $Id: MaskC0.cc,v 1.2 2006/05/21 00:35:13 adrian Exp $
// $Name: mokka-07-00 $
//
// History:
...
G4bool MaskC0::ContextualConstruct(const CGAGeometryEnvironment &env, G4LogicalVolume *worldLog)
{
  ...
  const G4double qd0_z_start = env.GetParameterAsDouble("Mask_QD0_z_start");
  const G4double qd0_z_end = env.GetParameterAsDouble("Mask_QD0_z_end");
  const G4double qd0_r_outer_start = env.GetParameterAsDouble("Mask_QD0_r_outer_start");
  const G4double qd0_r_outer_end = env.GetParameterAsDouble("Mask_QD0_r_outer_end");
  ...
  while (componentsDB->getTuple()) {
    ...
    if (offsetValue != 0) {
      G4cout
        << "MaskC0: Using " << globalName << " = "
        << currentValue / mm << " mm instead of "
        << assumedValue / mm << " mm" << G4endl;
      usingOffsets = true;
    }
  }
  if (usingOffsets) Control::Log("MaskC0: Be sure you know what you're doing!");
  ...
  while (componentsDB->getTuple()) { // build each single component with...
    ...
    while (consDB->getTuple()) {
      ...
      G4double zStart       = consDB->fetchDouble("zStart")      * mm + zStartOffset;
      G4double zEnd         = consDB->fetchDouble("zEnd")        * mm + zEndOffset;
      G4double rInnerStart  = consDB->fetchDouble("rInnerStart") * mm + rInnerStartOffset;
      G4double rInnerEnd    = consDB->fetchDouble("rInnerEnd")   * mm + rInnerEndOffset;
      G4double rOuterStart  = consDB->fetchDouble("rOuterStart") * mm + rOuterStartOffset;
      G4double rOuterEnd    = consDB->fetchDouble("rOuterEnd")   * mm + rOuterEndOffset;
      const G4String materialName = consDB->fetchString("material");
      const G4String volName      = "mask_" + consDB->fetchString("name");

      // if the volume name equal mask_QD0, update the related parameters.
      if (volName == "mask_QD0") {
	zStart = qd0_z_start;
	zEnd = qd0_z_end;
	rOuterStart = qd0_r_outer_start;
	rOuterEnd = qd0_r_outer_end;
      }
      ...
      if (crossingAngle == 0 && crossType != kCenter) {
        Control::Log("MaskC0: You are trying to build a crossing geometry without a crossing angle.\n"
      ...
      switch (crossType) {
        ...
        default: {
          Control::Log("MaskC0: Unimplemented \"crossType\" code.");
