...
// - TPC09: fixed ionisation threshold -- Martin Killenberg, 2010-02-03
...
G4bool TPC09::ContextualConstruct(const CGAGeometryEnvironment &env, G4LogicalVolume *worldLog)
{
  ...
  zAnode = dzTotal - dzEndplate; // the edge of the readout terminating the drift volume
 ...
 new G4PVPlacement(0, G4ThreeVector(0.,0.,-(dzCathode-cathode_mylar)/2+cathode_cupper), cathodeMyLog, "TPCCathodePiece", cathodeLog, false, 1);
  ...
  // No, 2*ionisation energy is the rule of thumb for the MEAN energy that is needed to create an
  // electron. The real value for Argon is 26 eV.
  // The THRESHOLD, however, certainly is the ionisation energy, i. e. 16 eV, so we need two variables.
  
  Ar_IonPotential=26* eV;
  G4double Ar_IonisationThreshold = 16*eV;
  
  TPCSD04 *sensitiveDetector = new TPCSD04("TPC", Ar_IonisationThreshold );
...
//  G4cout << "the outer layer of the TPC = " <<  rOuterSensitive << G4endl;
  ...
  while (db->getTuple()) {
    ...
    zCursor += dzPiece;
    if (zCursor > +dzEndplate / 2) Control::Abort("TPC09: Overfull TPC endplate.",MOKKA_ERROR_BAD_DATABASE_PARAMETERS);
  }
...
new G4PVPlacement(0, G4ThreeVector(0, 0, zCursor + dzPiece / 2), pieceLog, "TPCEndplateEndPiece", endplateLog, false, 0);
  ...
  G4cout << "TPC09: Inner radius of the sensitive volume is " << std::setw(4) << rInnerSensitive / mm << " mm." << G4endl;
  G4cout << "TPC09: Outer radius of the sensitive volume is " << std::setw(4) << rOuterSensitive / mm << " mm." << G4endl;
  G4cout << "TPC09: Number of Pad Rows in the TPC  " << std::setw(4) << numberPadRows << G4endl;
  G4cout << "TPC09: Limiting the step length in the TPC to  " << std::setw(4) << TPCMaxStepLength / mm << " mm." << G4endl;
  G4cout << "TPC09: Endplate material corresponds to " << G4int(fracRadLength * 1000) / 10.0 << "% of a radiation length." << G4endl;
...
{ 
  ...
  tpcParameters -> setDoubleVal( "zAnode", zAnode);
