...
// *********************************************************
//
// $Id: Yoke04.cc,v 1.3 2008/10/31 15:41:52 frank Exp $
// $Name: mokka-07-00 $
//
// History:  
...
// - added barrelEndcapGap, gear parameters, made barrel and endcap same thickness, made plug insensitive,  F.Gaede, DESY 2008-10-04
//
#include "Yoke04.hh"
...
G4bool Yoke04::ContextualConstruct(const CGAGeometryEnvironment &env, G4LogicalVolume *worldLog)
{
  ...
  // fg:  get rid of barrel_half_z and make endcap and barrel same thickness
 ...
 // const G4double yokeThickness             = zEndYoke - zStartEndcap;
  ...
  G4double testbarrel= number_of_layers*(iron_thickness+layer_thickness); 
   if( testbarrel >= yokeThickness  || testbarrel >= ( zEndYoke-zStartEndcap))
      Control::Abort("Yoke04: Overfull barrel or endcap! check layer number or dimensions !  sorry :( ",MOKKA_ERROR_BAD_DATABASE_PARAMETERS);
	...
	if( i!=number_of_layers)
	  {
	   ...
	   barrelParam->layerLayout().addLayer( iron_thickness +layer_thickness, cell_dim_x, cell_dim_z , iron_thickness ) ;
#endif		
	  ...
	  // that doesn't depend on the number of layers inside 
	   ...
	   radius_sensitive = radius_low+shift_in_layer;
#ifdef MOKKA_GEAR
	   double abs_thickness = yokeThickness - number_of_layers * (iron_thickness +layer_thickness) - layer_thickness ;
	   barrelParam->layerLayout().addLayer( abs_thickness +layer_thickness, cell_dim_x, cell_dim_z , abs_thickness ) ;
#endif		
	  }
...
// fg:   introduce #layers in plug
 unsigned int nLayerPlug = 0 ; // ILD plug is 100mm Fe - no instrumentation !
 
 if (plug_thickness<0.0*mm || plug_thickness < nLayerPlug*layer_thickness) 
  Control::Abort("Yoke04: Plug thickness negative or thinner then nLayerPlug layers of rpc ! \n Check the geometry! Sorry for abort :( ",MOKKA_ERROR_BAD_DATABASE_PARAMETERS);
...
#ifdef MOKKA_GEAR
	   //fg: gear layers include absorbers and sensitive parts - here layer_thickness is just sensitive ...
	   plugParam->layerLayout().addLayer(  plug_step + layer_thickness, cell_dim_x, cell_dim_z , plug_step ) ;
#endif		
...
G4LogicalVolume * 
Yoke04::BuildRPC1Box(G4Box* ChamberSolid, muonSD* theSD, G4int layer_id, G4UserLimits* pULimits, Database *db)
{
  ...
  if(ChamberSolid->GetEntityType()=="G4Box")
	...
	if (zCursor > +ChamberSolid->GetYHalfLength()/ 2) 
	  Control::Abort("Yoke04:Overfull RPC layer! sorry :( ",MOKKA_ERROR_BAD_DATABASE_PARAMETERS);
      }
      
      return ChamberLog;  
    }

  Control::Abort("Yoke04::BuildRPC1Box: invalid ChamberSolidEnvelope",MOKKA_OTHER_ERRORS);
  return NULL;
}


G4LogicalVolume * Yoke04::BuildRPC1ECShape(G4Polyhedra* ChamberSolid,muonSD* theSD,
  ...
  Control::Abort("Yoke04::BuildRPC1ECLayer: invalid ChamberSolidEnvelope",MOKKA_OTHER_ERRORS);
  return NULL;

}

G4LogicalVolume * Yoke04::BuildRPC1PlugShape(G4Polyhedra* ChamberSolid,muonSD* theSD,
					   G4int layer_id,G4UserLimits* pULimits, Database *db,
					   const CGAGeometryEnvironment &env)
  { 
 ...
 if(ChamberSolid->GetEntityType()=="G4Polyhedra")
  ...
  Control::Abort("Yoke04::BuildRPC1PlugLayer: invalid ChamberSolidEnvelope",MOKKA_OTHER_ERRORS);
  return NULL;
}
