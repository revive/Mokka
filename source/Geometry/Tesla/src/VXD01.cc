...
// *********************************************************
//
// $Id: VXD01.cc,v 1.14 2008/10/06 12:58:35 musat Exp $
// $Name: mokka-07-00 $
//
// History:
...
G4bool VXD01::construct(const G4String &dbName, G4LogicalVolume *worldLog)
{
  ...
  side_band_electronics=
    db->fetchInt("side_band_electronics");
  ...
  G4double  shell_inner_radious,shell_half_z,
    ...
    offset_ladder_block, beryllium_ladder_block_length ,
  ...
  do
    {
      G4int LayerId;
      G4double Z;
      G4double layer_radius, ladder_length, ladder_width ,ladder_gap,
	strip_line_final_z, nb_ladder, phirot,phirot2,
...
// ****************************************************************************************
	...
	= new G4Box("LadderSupport",
		     ladder_width+(side_band_electronics*end_electronics_width),
		     ladder_length,
                     support_structure_thickness/2.);

      VisAtt = new G4VisAttributes(G4Colour(1.,0.,0.,1.));
...
//       fprintf(stdout, "support layer =%d : ladder = %f , phirot =%f , phirot2 =%f ,offset_phi=%f \n", LayerId , support_loop, phirot , phirot2, offset_phi);
      ...
      thisLadder.width     = (ladder_width+(side_band_electronics*end_electronics_width))*2 ;
      thisLadder.radLength = (LadderSupportLogical->GetMaterial())->GetRadlen()/mm ;
      
      // find out type
      if( side_band_electronics == 0 ) gearHelpType = gear::ZPlanarParametersImpl::CCD  ;
      if( side_band_electronics == 1 ) gearHelpType = gear::ZPlanarParametersImpl::CMOS ;
...
for (G4double AnnulusBlock_loop=0;AnnulusBlock_loop<nb_ladder;AnnulusBlock_loop++) {
	...
	ZAnnulusBlock= ladder_length + end_electronics_half_z -( beryllium_ladder_block_length*side_band_electronics);

      Phys=
	new G4PVPlacement(rot,
			  G4ThreeVector((layer_radius-beryllium_ladder_block_thickness+((1-side_band_electronics)*support_structure_thickness))*sin(phirot2)+offset_phi*cos(phirot2),
			               -(layer_radius-beryllium_ladder_block_thickness+((1-side_band_electronics)*support_structure_thickness))*cos(phirot2)+offset_phi*sin(phirot2),
			                 ZAnnulusBlock),
...
//****************************************************************************************
	...
	= new G4Box("ElectronicsEnd",
		     end_electronics_width,
		     (ladder_length*(side_band_electronics/2.))+end_electronics_half_z,
		     electronics_structure_thickness/2.);

      VisAtt = new G4VisAttributes(G4Colour(0.,1.,0.,1.));
      //VisAtt->SetForceWireframe(true);
      //VisAtt->SetForceSolid(true);
      VisAtt->SetDaughtersInvisible(true);
	...
	new G4LogicalVolume(ElectronicsEndSolid,
     ...
     G4double electronic_offset_phi = offset_phi - (side_band_electronics * ladder_width);
   ...
   for (G4double elec_loop=0; elec_loop<nb_ladder;elec_loop++) {
	...
	Z= (ladder_length* (1-side_band_electronics/2.)) + end_electronics_half_z + ladder_gap/2.;

      Phys=
	new G4PVPlacement(rot,
			  G4ThreeVector((layer_radius+support_structure_thickness+(electronics_structure_thickness/2.))*sin(phirot2)+electronic_offset_phi*cos(phirot2),
			               -(layer_radius+support_structure_thickness+(electronics_structure_thickness/2.))*cos(phirot2)+electronic_offset_phi*sin(phirot2),
			                 Z),
...
//****************************************************************************************
      ...
      G4double strip_line_start_z = ladder_length + ladder_gap/2. + end_electronics_half_z * 2.+ shell_thickess; // to avoid overlaps
...
//****************************************************************************************
	...
	= new G4Box("SiActiveLayer",
      ...
      VisAtt = new G4VisAttributes(G4Colour(0.,1.,1.,1.));
      //VisAtt->SetForceWireframe(true);
      //VisAtt->SetForceSolid(true);
      VisAtt->SetDaughtersInvisible(true);
	...
	new G4LogicalVolume(SiActiveLayerSolid,
     ...
     G4double active_offset_phi = offset_phi + (side_band_electronics * end_electronics_width);
  ...
  for (G4double active_loop=0;active_loop<nb_ladder;active_loop++){
	...
	rot->rotateY(phirot2);
      ...
      Phys=
	new G4PVPlacement(rot,
			  G4ThreeVector((layer_radius+support_structure_thickness+(active_silicon_thickness/2.))*sin(phirot2)+active_offset_phi*cos(phirot2),
                                       -(layer_radius+support_structure_thickness+(active_silicon_thickness/2.))*cos(phirot2)+active_offset_phi*sin(phirot2),
				         Z),
  ...
  // sensitive layer
  helpLayer thisSens ;
  thisSens.distance  = layer_radius + support_structure_thickness ;
  ...
  ZEndPlateShell2 = 50. +  end_electronics_half_z * 2 + shell_thickess/2. ;
