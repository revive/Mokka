...
// *********************************************************
//
// $Id: TPC07.cc,v 1.1 2009/05/13 08:22:57 steve Exp $
// $Name: mokka-07-00 $
//
// History:  
...
G4bool TPC07::ContextualConstruct(const CGAGeometryEnvironment &env, G4LogicalVolume *worldLog)
{
 ...
 new G4PVPlacement(0, G4ThreeVector(0.,0.,-(dzCathode-cathode_mylar)/2+cathode_cupper), cathodeMyLog, "TPCCathodePiece", cathodeLog, false, 1);
  ...
  TPCSD03 *sensitiveDetector = new TPCSD03("TPC", Ar_IonPotential, Control::TPCCut);
...
//  G4cout << "the outer layer of the TPC = " <<  rOuterSensitive << G4endl;
  ...
  while (db->getTuple()) {
    ...
    zCursor += dzPiece;
    if (zCursor > +dzEndplate / 2) Control::Abort("TPC07: Overfull TPC endplate.",MOKKA_ERROR_BAD_DATABASE_PARAMETERS);
  }
...
new G4PVPlacement(0, G4ThreeVector(0, 0, zCursor + dzPiece / 2), pieceLog, "TPCEndplateEndPiece", endplateLog, false, 0);
  ...
  G4cout << "TPC07: Inner radius of the sensitive volume is " << std::setw(4) << rInnerSensitive / mm << " mm." << G4endl;
  G4cout << "TPC07: Outer radius of the sensitive volume is " << std::setw(4) << rOuterSensitive / mm << " mm." << G4endl;
  G4cout << "TPC07: Number of Pad Rows in the TPC  " << std::setw(4) << numberPadRows << G4endl;
  G4cout << "TPC07: Limiting the step length in the TPC to  " << std::setw(4) << TPCMaxStepLength / mm << " mm." << G4endl;
  G4cout << "TPC07: Endplate material corresponds to " << G4int(fracRadLength * 1000) / 10.0 << "% of a radiation length." << G4endl;
