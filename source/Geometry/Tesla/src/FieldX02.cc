...
// ********************************************************
//
// $Id: FieldX02.cc,v 1.1 2008/12/05 17:19:36 mora Exp $
// $Name: mokka-07-00 $
//
// History:
...
// - modified for scaling geometries as FieldX02: Adrian Vogel, 2006-04-20
...
INSTANTIATE(FieldX02)

G4bool FieldX02::ContextualConstruct(const CGAGeometryEnvironment &env, G4LogicalVolume *worldLog)
{
  ...
  while (db->getTuple()) {
    ...
    if (offsetValue != 0) {
      G4cout
        << "FieldX02: Using " << globalName << " = "
        << currentValue / mm << " mm instead of "
        << assumedValue / mm << " mm" << G4endl;
      usingOffsets = true;
    }
  }
  if (usingOffsets) Control::Log("FieldX02: Be sure you know what you're doing!");
  ...
  while (db->getTuple()) {
    ...
    if (_crossingAngle == 0 && (region.fieldType == kUpstreamQuad || region.fieldType == kDnstreamQuad || region.fieldType == kDID || region.fieldType == kMapDID)) {
      Control::Log("FieldX02: You are trying to build a crossing geometry without a crossing angle.\n"
    ...
    switch (region.fieldType) {
      ...
      case kMapSolenoid:
        ...
        region.fieldMap = new FieldX02MapSolenoid(db->fetchString("fieldData").data(),modelBFactor);
        break;
      case kMapDID:
        region.fieldValue = db->fetchDouble("fieldValue"); // scaling factor for given data
        region.fieldMap = new FieldX02MapDID(db->fetchString("fieldData").data(),modelBFactor);
        break;
      default:
        Control::Log("FieldX02: Unimplemented \"fieldType\" code.");
...
void FieldX02::GetFieldValue(const double point[4], double *bField) const
{
...
FieldX02VMap::FieldX02VMap(G4String tableName, G4double theModelBFactor)
{
...
FieldX02VMap::~FieldX02VMap(void)
{
}

void FieldX02MapSolenoid::GetFieldValue(const G4ThreeVector &pos, G4ThreeVector &value)
{
...
void FieldX02MapDID::GetFieldValue(const G4ThreeVector &pos, G4ThreeVector &value)
{
  const G4int signZ = (pos.getZ() >= 0) ? (+1) : (-1); // neglect the case z = 0 here...
