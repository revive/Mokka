...
//#define SHCALSC01_DEBUG

INSTANTIATE(SHcalSc01)


G4bool SHcalSc01::ContextualConstruct(const CGAGeometryEnvironment 
...
// Hcal  endcap modules
theENDCAPEndSD = new SDHcalEndCap(Hcal_cell_dim_x,
...
//----------------------------------------------------
// EndCap Rings 
if(Hcal_ring > 0) EndcapRings(WorldLog);
//----------------------------------------------------
...
//dblParam = gearMgr->tmpParam.getDoubleVal( "" );
...
//~              Barrel
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
void SHcalSc01::Barrel(G4LogicalVolume* MotherLog)
{
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
void SHcalSc01::BarrelRegularModules(G4LogicalVolume* MotherLog)
{
// Regular modules
...
EnvLogHcalModuleBarrel  = new G4LogicalVolume(ModuleSolid,
						RadiatorMaterial,//iron or tungsten
						"barrelHcalModule", 
						0, 0, 0);
...
else Control::Abort("Invalid sensitive model for the chosen HCAL superdriver!",MOKKA_ERROR_BAD_GLOBAL_PARAMETERS);
...
//~              Endcaps                              ~
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
void SHcalSc01::Endcaps(G4LogicalVolume* MotherLog)
{
...
G4cout<<G4endl;
#ifdef SHCALSC01_DEBUG
  G4cout<<"    Endcaps             : Rinner="<<pRMin<<"mm Router="<<pRMax<<G4endl;
#endif
...
helpEndcap.outerRadius = rOuter[0];  // outer radius
...
G4Polyhedra *EndCapSolid = new G4Polyhedra("HcalEndCapSolid",
					     0.,  //phiStart
					     360.,//phiTotal
					     8,   //numSide
					     2,   //numZPlanes
...
//
//=======================================================================
void SHcalSc01::EndcapChambers(G4LogicalVolume* MotherLog, SDHcalEndCap* theSD, bool rings)
{
// Chambers in the SHcalSc01::Endcaps
...
//
//======================================================================
void SHcalSc01::LoadEvent(FILE* theSubDetectorEventHitsFileInput)
{
...
//
//======================================================================
G4bool SHcalSc01::Setup(const CGAGeometryEnvironment &theGeometryEnvironment)
{
...
if(Hcal_barrel_end_module_type == 1 )
...
else
  {
    ...
    G4double total_z_size = 140 + 2 * TPC_Ecal_Hcal_barrel_halfZ;
    G4double Hcal_normal_dim_z = total_z_size / 5. * 1080./1120.;
    G4double Hcal_top_end_dim_z = (total_z_size - 3 * Hcal_normal_dim_z)/2;
    G4cout << "regular barrel module z size = " << Hcal_normal_dim_z
	     << ", end barrel z size = " << Hcal_top_end_dim_z << G4endl;
    Hcal_start_z = 1.5 * Hcal_normal_dim_z + Hcal_top_end_dim_z + 
	2 * Hcal_modules_gap + Hcal_endcap_cables_gap;
  }
...
Hcal_y_dim2_for_z = (Hcal_top_end_dim_z-Hcal_normal_dim_z) * 0.8179077682; // tan(RADIANS(39.28))
Hcal_y_dim3_for_z = Hcal_total_dim_y - Hcal_y_dim1_for_z - Hcal_y_dim2_for_z;
...
if( Hcal_barrel_end_module_type != 1)
  Control::Abort("SHcalSc01: Sorry, but Tesla like end modules in barrel not yet available with this driver.",MOKKA_ERROR_BAD_GLOBAL_PARAMETERS);
...
//~              EndcapRings                          ~
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
void SHcalSc01::EndcapRings(G4LogicalVolume* MotherLog)
{
// old parameters from database
G4double pRMax, pDz, pRMin;
pRMax = Hcal_endcap_rmax;
...
stop_z = start_z + MaxNumberOfLayers * (Hcal_chamber_thickness + Hcal_radiator_thickness)
 ...
 G4cout << "    HCAL endcap rings ZMax = " << stop_z << G4endl;
...
pRMin = Ecal_endcap_outer_radius
 ...
 G4cout << "    HCAL endcap rings Rmin = " << pRMin  
	<< "    HCAL endcap rings Rmax = " << pRMax
	<< G4endl;
...
if (rOuter[0] <= rInner[0]) 
  G4Exception("SHcalSc01::EndcapRings() - not enough place for endcap rings (try a larger Hcal_nlayers number)!");
...
//
//=========================================================================
G4bool SHcalSc01::PostConstructAction(CGAGeometryEnvironment& )
{
//
// Propagates the changes to Coil, if any. The SHcal has also the responsability 
// to change the calorimeter region parameters.
//
G4double Hcal_R_max = (Hcal_y_dim1_for_x +  Hcal_y_dim2_for_x + Hcal_inner_radius)/cos(pi/8);
...
G4double calorimeter_region_zmax = Hcal_start_z + Hcal_total_dim_y;
std::ostringstream oss3;  
oss3 << calorimeter_region_zmax;
(*Control::globalModelParameters)["calorimeter_region_zmax"] = oss3.str();
			    
 G4cout << "SHcalSc01 information: Hcal_outer_radius = "
	<< Hcal_outer_radius
	<< "\n                       module thickness = "
	<< Hcal_total_dim_y
	<< "\n                       Hcal_R_max = "
...
//
//===========================================================================
void SHcalSc01::CalculateFractTileSize(G4double x_length, G4double x_integerTileSize, G4double &x_fractionalTileSize)
{
...
//
//==================================================
void SHcalSc01::BarrelModuleGap(G4LogicalVolume *MotherLog)
{
...
//
//==============================================================================================
void SHcalSc01::BarrelHalfRegularChambersTrap(G4LogicalVolume *MotherLog, 
...
for (G4int layer_id = 1; layer_id <= (2*Hcal_nlayers); layer_id++){
  ...
  G4double scintHalfWidth = y_height - Hcal_fiber_gap/2. ;
  // fiber gap can't be larger than total chamber
  if (scintHalfWidth <= 0.) G4Exception("SHcalSc01::BuildLefBarrelChambers() - scintHalfWidth invalid!");
...
//
//========================================================================
void SHcalSc01::BarrelChambersSupportTrap(G4LogicalVolume *MotherLog,
					G4double chambers_y_off_correction)
{
...
//
//========================================================================
void SHcalSc01::BarrelChambersGap(G4LogicalVolume *MotherLog,
				  G4double chambers_y_off_correction)
{
...
//
//========================================================================
void SHcalSc01::CalculateXLayer(G4int layer_id, G4int &logical_layer_id,
...
if(logical_layer_id *(Hcal_radiator_thickness + Hcal_chamber_thickness) < Hcal_y_dim1_for_x ){
  ...
  x_total  = Hcal_midle_dim_x/2. - Hcal_middle_stave_gaps/2. - Hcal_back_plate_thickness - xOffset;
  x_length = x_total - 2.*Hcal_layer_support_length  - 2.* Hcal_layer_air_gap;
  x_halfLength = x_length/2.;
