...
//*******************************************************
//
// $Id: SHcal04.cc,v 1.16 2009/02/05 17:27:05 musat Exp $
// $Name: mokka-07-00 $
//
//
// History:  
...
G4bool 
SHcal04::ContextualConstruct(const CGAGeometryEnvironment 
			     ...
			     G4LogicalVolume *WorldLog)
{
  ...
  if(Hcal_radiator_material == "Iron")
    ...
    if(Hcal_radiator_material == "stainless_steel")
      RadiatorMaterial =  CGAGeometryManager::GetMaterial("stainless_steel");
    else 
      Control::
	Abort("SHcal04: invalid radiator material name. \nIt has to be either Iron or stainless_steel!!!",MOKKA_ERROR_BAD_GLOBAL_PARAMETERS);
...
void SHcal04::BuildMaterials(void) {
...
//~              Barrel
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
void SHcal04::Barrel(G4LogicalVolume* MotherLog)
{
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
void SHcal04::BarrelVirtualModules(G4LogicalVolume* MotherLog)
{
...
if((halfZ-RPC_EdgeWidth)<=0.)
	Control::Abort("SHcal04:Too short layer",MOKKA_OTHER_ERRORS);
#endif
...
//~              Endcaps                 ~
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
void SHcal04::Endcaps(G4LogicalVolume* MotherLog)
{
...
void SHcal04::EndcapChambers(G4LogicalVolume* MotherLog,
			     SDHcalECSD01* theSD, bool rings)
{
  ...
  {
    // Chambers in the SHcal04::Endcaps
  ...
  // standard endcap chamber logical
    ...
    possible_number_of_chambers = (G4int) (floor(abs(Hcal_total_dim_y) /
	  (Hcal_chamber_tickness + Hcal_radiator_thickness)));

    zValue = Hcal_total_dim_y/2.;
...
G4LogicalVolume * 
SHcal04::BuildRPC2Box(G4Box* ChamberSolid, 
		       SDHcalSD01* theSD, 
		       G4int layerId,
		       G4UserLimits* pULimits)
{
...
*/
	...
	new G4LogicalVolume(ChamberSolid,
      ...
      //
      G4VSolid *solidRPCSpacer=new G4Tubs("tube",0,RPC_Gap_Thickness/2,RPCSpacerLength/2,0,2*pi);
...
,true);
  ...
  Control::Abort("SHcal04::BuildRPC2: invalid ChamberSolidEnvelope",
	MOKKA_OTHER_ERRORS);
  return NULL;
}

G4LogicalVolume * 
SHcal04::BuildRPC2Polyhedra(G4VSolid* ChamberSolid, 
			   SDHcalECSD01* theSD,
			   bool rings,
			   G4double phiStart,
			   G4double phiTotal,
			   ...
			   G4UserLimits* pULimits)
{
   ...
   {
	 Control::Abort("SHcal04::BuildRPC2Polyhedra: invalid ChamberSolidEnvelope for the Hcal Rings", MOKKA_OTHER_ERRORS);
         return NULL;
...
void 
SHcal04::LoadEvent(FILE* theSubDetectorEventHitsFileInput)
{
...
G4bool 
SHcal04::Setup(const CGAGeometryEnvironment &theGeometryEnvironment)
{
  Hcal_radiator_thickness =
...
//~              EndcapRings                 ~
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
void SHcal04::EndcapRings(G4LogicalVolume* MotherLog)
{
  // old parameters from database
  G4double pRMax, pDz, pRMin;
  pRMax = Hcal_endcap_rmax;
...
G4bool 
SHcal04::PostConstructAction(CGAGeometryEnvironment& )
{
  //
