		    ...
		    G4LogicalVolume *theWorld)
{
  ...
  G4cout << "\nBuilding Ecal- SEcal04"<< G4endl;
  
  // Initialize the Geant3 interface
  if(Control::DUMPG3) MyPlacement::Init("ECAL","SEcal04");
...
G4bool 
SEcal04::Setup(const CGAGeometryEnvironment &theGeometryEnvironment)
{
  ...
  // Calculed parameters.
  // Ecal_total_SiSlab_thickness = just one Slab side in the H, from 
  // shielding to mass

  total_number_of_layers =
    Ecal_nlayers1 + Ecal_nlayers2 + Ecal_nlayers3;
  if((total_number_of_layers % 2) == 0)
    Control::Abort("SEcal04: total number of layers has to be odd!",MOKKA_ERROR_BAD_GLOBAL_PARAMETERS);
  ...
  Ecal_Slab_Sc_PCB_thickness =
    theGeometryEnvironment.GetParameterAsDouble("Ecal_Slab_Sc_PCB_thickness");
  Ecal_Sc_thickness =
    theGeometryEnvironment.GetParameterAsDouble("Ecal_Sc_thickness");
  Ecal_Sc_reflector_thickness =
    theGeometryEnvironment.GetParameterAsDouble("Ecal_Sc_reflector_thickness");
  Ecal_MPPC_size =
    theGeometryEnvironment.GetParameterAsDouble("Ecal_MPPC_size");
  Ecal_Sc_MPPC_breadth =
    theGeometryEnvironment.GetParameterAsDouble("Ecal_Sc_MPPC_breadth");

  Ecal_Sc_N_strips_across_module =
    theGeometryEnvironment.GetParameterAsInt("Ecal_Sc_N_strips_across_module");
  if(Ecal_Sc_N_strips_across_module == 0)
	Control::Abort("SEcal04: Parameter \"Ecal_Sc_N_strips_across_module\" must be different from zero",MOKKA_ERROR_BAD_GLOBAL_PARAMETERS);

  Ecal_Sc_number_of_virtual_cells =
    theGeometryEnvironment.GetParameterAsInt("Ecal_Sc_number_of_virtual_cells");
  if(Ecal_Sc_number_of_virtual_cells == 0)
	Control::Abort("SEcal04: Parameter \"Ecal_Sc_number_of_virtual_cells\" must be different from zero",MOKKA_ERROR_BAD_GLOBAL_PARAMETERS);

  
  Ecal_Barrel_Sc_Si_Mix = 
    theGeometryEnvironment.GetParameterAsString("Ecal_Sc_Si_mix");
  ...
  G4cout << " Ecal_total_ScSlab_thickness = " << Ecal_total_ScSlab_thickness  << G4endl;
	...
	Control::Abort("SEcal04: The size of Ecal_Barrel_Sc_Si_Mix is not consistent with the total number of layers",MOKKA_ERROR_BAD_GLOBAL_PARAMETERS);
	...
	switch(Ecal_Barrel_Sc_Si_Mix[i_char]) {
		...
		case ECAL_SC_LAYER_1_ALONG_Z_LAYER_2_ALONG_X:
			...
			break;
		default:
			Control::Abort("SEcal04: The Ecal_Barrel_Sc_Si_Mix parameter should contain only 0's, 1's, 2's, 3's or 4's",MOKKA_ERROR_BAD_GLOBAL_PARAMETERS);
	};
...
G4bool SEcal04::Build(G4LogicalVolume* WorldLog)
{
 ...
 if(Number_of_Si_Layers_in_Barrel != 0)
 {
  ...
  barrelStripSizeParallelToZ = alveolus_dim_z / 
				Ecal_Sc_N_strips_across_module;
 
  virtualCellDim = barrelStripSizeParallelToZ /
                        Ecal_Sc_number_of_virtual_cells;

 if(Number_of_Sc_Layers_in_Barrel != 0)
 {
  theBarrelScintillatorSD = 
    new SEcalSD03(barrelStripSizeinX,
...
// interger number os cells in the given Si plates
  ...
  G4Tubs * CenterECTubForSi =
    new G4Tubs ("CenterECTubForSi",
                0.,
                Lcal_outer_radius + Ecal_Lcal_ring_gap
...
//   //----------------------------------------------------
  ...
  if(Number_of_Si_Layers_in_EC != 0)
  	EC_Initialize(EC_SiliconTowerSlabs,'0');

  if(EC_Scint_Along_X)
  	EC_Initialize(EC_ScintillatorParallelToXTowerSlabs,'x');

  if(EC_Scint_Along_Z)
  	EC_Initialize(EC_ScintillatorParallelToZTowerSlabs,'z');
 ...
 if(Number_of_Si_Layers_in_EC != 0)
    theEndCapSiliconSD->SetModuleZOffset(6,EC_module_z_offset);  
  
 if(Number_of_Sc_Layers_in_EC != 0)
    theEndCapScintillatorSD->SetModuleZOffset(6,EC_module_z_offset);
...
//GM:
      layerCode = Ecal_Barrel_Sc_Si_Mix[(unsigned int)((i-1)/2)];      
      getCellDims(cell_dim_1, cell_dim_2, i, layerCode);
//GM
      barrelParam->layerLayout().positionLayer
	(0, calcThick, cell_dim_1, cell_dim_2, 
...
//GM:
      layerCode = Ecal_Barrel_Sc_Si_Mix[(unsigned int)((helpBarrel.count-1)/2)];      
      getCellDims(cell_dim_1, cell_dim_2, helpBarrel.count, layerCode);
//GM
  barrelParam->layerLayout().positionLayer
    (0, calcThick, cell_dim_1, cell_dim_2, 
...
//GM:
      layerCode = Ecal_EndCap_Sc_Si_Mix[(unsigned int)((i-1)/2)];      
      getCellDims(cell_dim_1, cell_dim_2, i, layerCode);
//GM
  ...
  // the last layer...
//GM:
      layerCode = Ecal_EndCap_Sc_Si_Mix[(unsigned int)((helpEndcap.count-1)/2)];      
      getCellDims(cell_dim_1, cell_dim_2, helpEndcap.count, layerCode);
//GM
  endcapParam->layerLayout().positionLayer
    (0, calcThick, cell_dim_1, cell_dim_2, 
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
void 
SEcal04::BarrelStandardModule(G4LogicalVolume* MotherLog)
{
...
if(Number_of_Si_Layers_in_Barrel != 0)
  theBarrelSiliconSD->SetStandardXOffset(X);
if(Number_of_Sc_Layers_in_Barrel != 0)
  theBarrelScintillatorSD->SetStandardXOffset(X);
  ...
  for (G4int stave_id = 1; stave_id < 9 ; stave_id++)
	...
	new MyPlacement(rot,
		...
		theBarrelSiliconSD->SetModuleZOffset(module_id,
			   			module_z_offset);
	     }
	     if(Number_of_Sc_Layers_in_Barrel != 0)
	     {
		theBarrelScintillatorSD->SetStaveRotationMatrix(stave_id,							phirot);
		theBarrelScintillatorSD->SetModuleZOffset(module_id,
			   			module_z_offset);
	     }
    }
...
//~              EndcapStandardModule                 ~
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
G4LogicalVolume* SEcal04::EndcapStandardModule(G4bool Zminus)
{
...
void 
SEcal04::EndcapRadiatorPlates(G4LogicalVolume*& EndCapRadiatorL1,
			      ...
			      G4LogicalVolume*& EndCapRingSlabRadiatorL3)
{
  ...
  G4Tubs * CenterECTubForRadiator =
    new G4Tubs ("CenterECTubForRadiator",
                0.,
                Lcal_outer_radius + Ecal_Lcal_ring_gap
		+G4GeometryTolerance::GetInstance()->GetSurfaceTolerance(),
                module_thickness,
...
void 
SEcal04::LoadEvent(FILE* theSubDetectorEventHitsFileInput)
{
		    ...
		    G4LogicalVolume* EnvLogEcalModuleBarrel)
{
  ...
  if(Ecal_Barrel_Sc_Si_Mix[(unsigned int)((layer_id-1)/2)] != ECAL_SI_LAYERS)
	   Slab_thickness = Ecal_total_ScSlab_thickness;
  alveolus_dim_y =
    2 * Slab_thickness +
    W_thick + 
  ...
  G4double Slab_Y_offset = 0;

  char layerCode = Ecal_Barrel_Sc_Si_Mix[(unsigned int)((layer_id-1)/2)];

  char FirstSlabDir='0', SecondSlabDir='0';

  if(layerCode == ECAL_SI_LAYERS) {//Silicon slabs
    FirstSlabLog = BuildSiliconSlab(alveolus_dim_x,
	      Ecal_total_SiSlab_thickness,
	      alveolus_dim_z,
	      theBarrelSiliconSD);
    SecondSlabLog = FirstSlabLog;
    Slab_Y_offset = Si_Slab_Y_offset;
  }
  else {//Scintillator slabs
    FirstSlabDir='x'; SecondSlabDir='x';//if ECAL_SC_LAYER_1_2_ALONG_X

    if( layerCode == ECAL_SC_LAYER_1_2_ALONG_Z) {
 	FirstSlabDir  = 'z';
	SecondSlabDir = 'z';
    }
    ...
    Slab_Y_offset = Sc_Slab_Y_offset;
  ...
  for (G4int i_tower = Ecal_barrel_number_of_towers;
      ...
      if (i_tower == Ecal_barrel_number_of_towers)
	{
	    ...
	    theBarrelSiliconSD->AddLayer(layer_id,
		     x_off - 
		     ((G4Box *)FirstSlabLog->GetSolid())->GetXHalfLength(),
		     Ecal_inner_radius + module_thickness/2 +
		     y_off - Slab_Y_offset,
		     z_tower_center - 
		     ((G4Box *)FirstSlabLog->GetSolid())->GetYHalfLength());
      ...
      y_floor += Slab_thickness + Ecal_fiber_thickness;
      ...
      if (i_tower == Ecal_barrel_number_of_towers)
	{
	  if(SecondSlabDir == '0') //Silicon Slab
	    theBarrelSiliconSD->AddLayer(layer_id + 1,
		     x_off - 
		     ((G4Box *)SecondSlabLog->GetSolid())->GetXHalfLength(),
		     Ecal_inner_radius + module_thickness/2 +
		     y_off + Slab_Y_offset,
		     z_tower_center - 
		     ((G4Box *)SecondSlabLog->GetSolid())->GetYHalfLength());
...
G4double 
SEcal04::GiveMeRadiatorThickness(G4int layer_id)
{
...
G4LogicalVolume* 
SEcal04::BuildRadiatorPlate(G4double radiator_dim_x,
			    G4double radiator_dim_y,
			    G4double radiator_dim_z)
{
...
G4LogicalVolume* 
SEcal04::BuildSiliconSlab(G4double slab_dim_x,
		   G4double slab_dim_y,
		   G4double slab_dim_z,
		   SEcalSD02 * theSD)
{
...
void
SEcal04::BuildECRingAlveolus (G4int layer_id,
			      G4double z_floor,
...
,true);
  ...
  // place Rad (in z_floor + Ecal_total_SiSlab_thickness +
...
G4double 
SEcal04::BuildECAlveolus (G4int layer_id,
			  G4double z_floor,
			  G4LogicalVolume* ECLog,
			    G4bool Zminus)
{
...
//***
  ...
  std::vector<G4LogicalVolume*>* EC_FirstLayerTowerSlabs = 
		&EC_SiliconTowerSlabs;
  std::vector<G4LogicalVolume*>* EC_SecondLayerTowerSlabs = 
		&EC_SiliconTowerSlabs;
  G4double Slab_Y_offset  = Si_Slab_Y_offset;
  G4double Slab_thickness = Ecal_total_SiSlab_thickness;
  SEcalSD02* theEndCapSD = theEndCapSiliconSD;

  char layerCode = Ecal_EndCap_Sc_Si_Mix[(unsigned int)((layer_id-1)/2)];

    if( layerCode == ECAL_SC_LAYER_1_2_ALONG_X) {
	Slab_Y_offset = Sc_Slab_Y_offset;
	Slab_thickness = Ecal_total_ScSlab_thickness;
	...
	theEndCapSD = theEndCapScintillatorSD;
    }
...
//***
  ...
  for (G4int i_stave = 1;
	...
	if(Number_of_Si_Layers_in_EC != 0)
		theEndCapSiliconSD
			->SetStaveRotationMatrix(i_stave,angle_module);
	if(Number_of_Sc_Layers_in_EC != 0)
		theEndCapScintillatorSD
			->SetStaveRotationMatrix(i_stave,angle_module);
      }
      for (unsigned int i_tower=0;
	   i_tower < EC_FirstLayerTowerSlabs->size();
	   i_tower++)
	{
	  //**********************
	  //if(i_tower > 0) break;
	  ...
	  pos[2] = z_floor + Slab_thickness /2;
	  ...
	  new MyPlacement(rot,
			  pos,
			  (*EC_FirstLayerTowerSlabs)[i_tower],
			  "FirstSlabEC",
			  ECLog,
			  false,
			  i_stave*100000 +
	  ...
	  if (i_stave == 1  
	    ...
	    theEndCapSD->
	      AddLayer(layer_id,
		       - pos[0]
		       - ((G4Box *)(*EC_FirstLayerTowerSlabs)[i_tower]
...
// 	    rot->rotateX(pi);
	  ...
	  new MyPlacement(rot,
			  pos,
			  (*EC_SecondLayerTowerSlabs)[i_tower],
			  "SecondSlabEC",
			  ECLog,
			  false,
			  i_stave*100000 +
...
,true);
	  ...
	  if (i_stave == 1  
	    ...
	    theEndCapSD->
	      AddLayer(layer_id+1,
		       - pos[0]
		       - ((G4Box *)(*EC_SecondLayerTowerSlabs)[i_tower]
			  ->GetSolid())->GetXHalfLength(),
		       pos[2]
		       + Slab_Y_offset,
		       pos[1]
...
G4bool SEcal04::EC_Initialize(std::vector<G4LogicalVolume*>& EC_TowerSlabs,
				char direction)
{  
...
//     - Ecal_lateral_face_thickness;
  ...
  while ( ( y_floor + EC_alveolus_dim_y) < EC_y_top )
      ...
      if(last_dim_x != EC_alveolus_dim_x)
	{
	  if(direction == '0')
	  	EC_TowerSlabs.
	    		push_back(BuildSiliconSlab(EC_alveolus_dim_y,
				Ecal_total_SiSlab_thickness,
				EC_alveolus_dim_x,
				theEndCapSiliconSD));
	  else
		EC_TowerSlabs.
			push_back(BuildScintillatorSlab(EC_alveolus_dim_y,
			              Ecal_total_ScSlab_thickness,
			              EC_alveolus_dim_x,
			              direction,
			              theEndCapScintillatorSD,
...
G4bool 
SEcal04::PostConstructAction(CGAGeometryEnvironment& theGeometryEnvironment)
{
...
void SEcal04::DefineMaterial(void) {
	...
	G4double cuThickness  = Ecal_Slab_copper_thickness + cuTracksThickness 
			+ Ecal_Slab_shielding;

        G4double g10Volumefraction = g10Thickness / (g10Thickness + cuThickness);


	G4Material * theG10Mat = 
		CGAGeometryManager::GetMaterial("g10");
	...
	G4double pcbMixDensity = (g10Density * g10Volumefraction +
				cuDensity *(1 - g10Volumefraction)
			      ) * g/cm3;

	siPCBMix = new G4Material(name="siPCBMix", pcbMixDensity, nel=2);

        G4double g10FractionMass = g10Density * g10Thickness /
	...
	G4Material * theKaptonMat = 
		CGAGeometryManager::GetMaterial("kapton");

	G4double kaptonDensity = (theKaptonMat->GetDensity())/(g/cm3);

	G4double kaptonVolumefraction = kaptonGroundOrHVThickness / 
			(kaptonGroundOrHVThickness + cuGroundOrHVThickness);

	G4double groundMixDensity = (kaptonDensity * kaptonVolumefraction +
				cuDensity *(1 - kaptonVolumefraction)
			      ) * g/cm3;

        groundMix = new G4Material(name="GroundOrHVMix", groundMixDensity, nel=2);

	G4double kaptonFractionMass = kaptonDensity * kaptonGroundOrHVThickness /
	...
	G4double ScCuTracksThickness = 0;
        G4double ScG10Thickness = Ecal_Slab_Sc_PCB_thickness - 
						ScCuTracksThickness;
	G4double ScCuThickness  = Ecal_Slab_copper_thickness 
			+ ScCuTracksThickness 
			+ Ecal_Slab_shielding;

        G4double ScG10Volumefraction = ScG10Thickness / 
				(ScG10Thickness + ScCuThickness);
	G4double ScPCBMixDensity = (g10Density * ScG10Volumefraction +
				cuDensity *(1 - ScG10Volumefraction)
			      ) * g/cm3;
...
G4LogicalVolume* 
SEcal04::BuildScintillatorSlab(G4double slab_dim_x,
		   G4double slab_dim_y,
		   G4double slab_dim_z,
		   char direction,
		   SEcalSD03 * theSD,
                   ...
                   G4double theStripSizeParallelToZ)
{
  ...
  if(slab_dim_z > slab_dim_x) {
	...
	rotateSensVol->rotateZ(-pi/2.);
  	...
  	sensitiveLog = 
		BuildSensitiveVolume(slab_dim_z,
	      		     slab_dim_x,
			     direction, theSD,
			     theStripSizeinX,
  	...
  	sensitiveLog = 
		BuildSensitiveVolume(slab_dim_x,
	      		     slab_dim_z,
			     direction, theSD,
			     theStripSizeinX,
                   	     theStripSizeParallelToZ);

  G4VisAttributes* SensVisAtt = 
	new G4VisAttributes(G4Colour(0,0,1));
  SensVisAtt->SetForceWireframe(true);
  SensVisAtt->SetVisibility(false);
  ...
  G4Box* ScPCBCuShieldSolid = 
    new G4Box("ScPCBCuShieldSolid", 
	      slab_dim_x/2,
	      slab_dim_z/2,
	      ScPCBCuShield_thickness/2);
  G4LogicalVolume* ScPCBCuShieldLog = 
    new G4LogicalVolume(ScPCBCuShieldSolid,
			scPCBMix,
			"ScPCBCuShieldLogical", 
			0, 0, 0);
  G4VisAttributes* ScPCBCuShieldVisAtt = 
    new G4VisAttributes(G4Colour(0.5,0.5,0));
  ScPCBCuShieldVisAtt->SetForceWireframe(true);
  ScPCBCuShieldVisAtt->SetVisibility(false);
...
G4LogicalVolume* SEcal04::BuildSensitiveVolume(G4double slab_dim_x,
	        			       G4double slab_dim_y,
					       char direction,
					       SEcalSD03 * theSD,
			                       G4double theStripSizeinX,
                   			       G4double theStripSizeParallelToZ)
{
  ...
  G4LogicalVolume * SensLog = 
    new G4LogicalVolume(SensSolid,
  ...
  G4LogicalVolume *nominalStripLogical = BuildStripVolume(theStripSizeinX
	- 6*( G4GeometryTolerance::GetInstance()->GetSurfaceTolerance() ) ,
                                        theStripSizeParallelToZ
	- 6*( G4GeometryTolerance::GetInstance()->GetSurfaceTolerance() ) , 
                                        sensThickness, theSD);
  ...
  if(direction == 'z')
  {
	...
	G4LogicalVolume *stripContainerLogical=
		new G4LogicalVolume(stripContainerBox,
                        CGAGeometryManager::GetMaterial("air"),
                        "ScZStripContainerLogical",
                        0,
                        0,
                        0);
...
/*
	new G4PVReplica("ScStripsAlongZ",
                          nominalStripLogical,
                          ...
                          theStripSizeParallelToZ,
                          0);
	//the copy number of the ScStripsAlongZ replica gives the
	//copy number along Z of the scintillator strip
*/

	//replacing replica with MyPlacement due to the problem seen
	//in the replica for direction == 'x'

	G4double yOff = 0;
	for(int i_strip = 0; i_strip < Ecal_Sc_N_strips_across_module; 
								i_strip++){
		yOff = -slab_dim_y/2 + theStripSizeParallelToZ/2 +
			i_strip * theStripSizeParallelToZ;
	...
	for(int i_container = 0; i_container < nStripContainersInX; 
			i_container++) {


        	new MyPlacement(0, G4ThreeVector(xOffSet, 0, 0),
                        stripContainerLogical,
                        "ZStripContainerPhysical",
                        SensLog,
                        false,
                        10000 + i_container + 1);
  ...
  else if(direction == 'x')
  {
	...
	int nNominalStripsInX = int(slab_dim_x / theStripSizeParallelToZ);
        G4double sizeOfLastStrip = slab_dim_x - 
				nNominalStripsInX * theStripSizeParallelToZ;
	if(sizeOfLastStrip <= theStripSizeParallelToZ/2) {
		nNominalStripsInX--;
		sizeOfLastStrip += theStripSizeParallelToZ;
	}
	
        G4LogicalVolume *lastStripLogical = BuildStripVolume(theStripSizeinX
		- 6*( G4GeometryTolerance::GetInstance()->GetSurfaceTolerance() ) ,
                                        sizeOfLastStrip
		- 6*( G4GeometryTolerance::GetInstance()->GetSurfaceTolerance() ) ,
                                        sensThickness, theSD);
	...
	for(int i_strip = 0; i_strip < nNominalStripsInX; i_strip++) {
		...
		xOffSet = -slab_dim_x/2 + theStripSizeParallelToZ/2
			+ i_strip * theStripSizeParallelToZ;
...
/*
	new G4PVReplica("ScXStripContainersAlongZ",
                          stripContainerLogical,
                          SensLog,
                          kYAxis,
                          2 * N_cells_in_Z,
                          theStripSizeinX,
                          0);
	//the copy number of the ScXStripContainersAlongZ replica gives the
	//copy number along Z of the scintillator strip
*/
	//replacing replica with MyPlacement due to the problem seen
	//in the replica for direction == 'x'
	...
	for(int i_cont = 0; i_cont < 2 * N_cells_in_Z; i_cont++) {
		yOffSet = -slab_dim_y/2 + theStripSizeinX/2 +
			i_cont * theStripSizeinX;
	  
		new MyPlacement(0, G4ThreeVector(0, yOffSet, 0),
                        stripContainerLogical,
                        "ScXStripContainersAlongZ",
                        SensLog,
                        false,
                        i_cont);
	}
	...
	Control::Abort("SEcal04: The direction of the scintillator strips can only be 'x' or 'z'",MOKKA_OTHER_ERRORS);

  return SensLog;
}


G4LogicalVolume* SEcal04::BuildStripVolume(G4double stripSizeinX,
					...
					SEcalSD03 * theSD) 
{
  ...
  G4LogicalVolume *stripLogical=
		new G4LogicalVolume(stripBox,
                        CGAGeometryManager::GetMaterial("mylar"),
                        "ReflectorStripLogical",
                        0,
                        0,
                        0);
  G4VisAttributes * VisAttReflector = 
			new G4VisAttributes(G4Colour(0.5,1.,1.));
  ...
  G4Box *scintillatorBox  = 
		new G4Box("ScintillatorStripSolid",
  ...
  if(Ecal_Sc_number_of_virtual_cells != 0)
    pULimits = new G4UserLimits(
			stripSizeParallelToZ/Ecal_Sc_number_of_virtual_cells);

  G4LogicalVolume *scintillatorLogical=
		new G4LogicalVolume(scintillatorBox,
                        CGAGeometryManager::GetMaterial("polystyrene"),
                        "ScintillatorStripLogical",
                        0, 0, pULimits);
  G4VisAttributes * VisAttScintillator = 
			new G4VisAttributes(G4Colour(0.5,0.5,1.));
  ...
  new MyPlacement(0, G4ThreeVector(0, 0, 0),
                        scintillatorLogical,
                        "ScintillatorStripPhysical",
                        stripLogical,
                        false,
                        0);
  ...
  G4LogicalVolume *mppcLogical =
		new G4LogicalVolume(mppcBox,
                        ...
                        0,
                        0);
  G4VisAttributes * VisAttMPPC = 
			new G4VisAttributes(G4Colour(0.5,0.5,1.));
  VisAttMPPC->SetForceWireframe(true);
  mppcLogical->SetVisAttributes(VisAttMPPC);

  G4double mppcZOffset = stripSizeParallelToZ/2 
			 - Ecal_Sc_reflector_thickness 
			 - Ecal_MPPC_size/2;

  new MyPlacement(0, G4ThreeVector(0, mppcZOffset, 0),
                        mppcLogical,
                        "MPPCPhysical",
                        scintillatorLogical,
                        false,
...
#ifdef MOKKA_GEAR
void SEcal04::getCellDims(G4double & cell_dim_1, G4double & cell_dim_2, int i, char layerCode)
{

      G4double cell_dim_11 = cell_dim_z;
      G4double cell_dim_12 = cell_dim_x;
      G4double cell_dim_21 = cell_dim_z;
      G4double cell_dim_22 = cell_dim_x;

      switch(layerCode) {
       case ECAL_SI_LAYERS:
          break;
       case ECAL_SC_LAYER_1_2_ALONG_X:
          cell_dim_11 = barrelStripSizeinX - 2*Ecal_Sc_reflector_thickness;
          cell_dim_12 = virtualCellDim;
          cell_dim_21 = barrelStripSizeinX - 2*Ecal_Sc_reflector_thickness;
          cell_dim_22 = virtualCellDim;
          break;
       case ECAL_SC_LAYER_1_2_ALONG_Z:
          cell_dim_11 = virtualCellDim;
          cell_dim_12 = barrelStripSizeinX - 2*Ecal_Sc_reflector_thickness;
          cell_dim_21 = virtualCellDim;
          cell_dim_22 = barrelStripSizeinX - 2*Ecal_Sc_reflector_thickness;
          break;
       case ECAL_SC_LAYER_1_ALONG_X_LAYER_2_ALONG_Z:
          cell_dim_11 = barrelStripSizeinX - 2*Ecal_Sc_reflector_thickness;
          cell_dim_12 = virtualCellDim;
