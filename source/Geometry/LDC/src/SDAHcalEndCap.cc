...
//*******************************************************
//
// $Id: SDAHcalEndCap.cc,2011.12.05 S.Lu $
// 
//
#include "Control.hh"
...
//                                                            ~
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    SDAHcalEndCap::SDAHcalEndCap(G4double Idim, G4double Jdim, G4double Thickness, 
...
#ifdef SDAHcalEndCap_DEBUG
  G4cout<<"HCAL endcap sensitive detector name: "<<SDname<<G4endl;
#endif
        int i;
	for(i=0;i<MAX_ENDCAP_MODULES_NUMBER;i++) ModulesYOffsets[i]=0;
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
G4ThreeVector SDAHcalEndCap::GetLocalCellCenter(G4int pI,G4int pJ,G4int pK, G4int pM) 
{
#ifdef SDAHcalEndCap_DEBUG
  G4cout<<"enter SDAHcalEndCap::GetLocalCellCenter I:J:K:M"<<G4endl;
#endif
  ...
  assert (pM >= 0);
  assert (pM <= MAX_ENDCAP_MODULES_NUMBER);
...
void SDAHcalEndCap::SetModuleYOffset(G4int moduleNumber,G4double Yoff)
{
	assert (moduleNumber<MAX_ENDCAP_MODULES_NUMBER && moduleNumber>=0);
...
//                                                            ~
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
G4bool SDAHcalEndCap::ProcessHits(G4Step *aStep,G4TouchableHistory *)
{
  ...
  if (theLayer <=0 || theLayer > MAX_LAYERS) 
    G4Exception("SDAHcalEndCap::ProcessHits() - BAD history, FATAL ERROR");
  ...
  if (I < 0 || I > 11) G4Exception("SDAHcalEndCap::ProcessHits() - invalid cell index I, aborting...");
  if (J < 0 || J > 96) G4Exception("SDAHcalEndCap::ProcessHits() - invalid cell index J, aborting...");
#endif

  //Must have I>=0
  if (I < 0) G4Exception("SDAHcalEndCap::ProcessHits() - invalid negativ cell index I, aborting...");
  
  //Must have J>=0
  if (J < 0) G4Exception("SDAHcalEndCap::ProcessHits() - invalid negativ cell index J, aborting...");
  ...
  // endCapID 7 and 8, there is the center hole. count all J from y = 0.0.
  // the J start from 12, [12, 96], other J: [0, MAX_itself]
  if(theModule == 7 || theModule ==8) J += 12;
