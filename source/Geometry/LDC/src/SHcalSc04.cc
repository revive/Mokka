...
//#define SHCALSC04_DEBUG
#define SHCAL_CHECK_OVERLAP 0

INSTANTIATE(SHcalSc04)

  G4bool SHcalSc04::ContextualConstruct(const CGAGeometryEnvironment 
					...
					G4LogicalVolume *WorldLog)
{
  ...
  */
  ...
  if( Hcal_nlayers > 64 || Hcal_endcap_nlayers > 64 ) 
    Control::Abort("SHcalSc04: The layer number  must smaller than 64 !\n M:3 S-1:3 I:9 J:9 K:6\n 8   8     512 512 64\n Please check Hcal_nlayers and Hcal_endcap_nlayers \n in the steering file",MOKKA_ERROR_BAD_GLOBAL_PARAMETERS);;
  ...
  else Control::Abort("SHcalSc04: Invalid sensitive model for the chosen HCAL superdriver!",MOKKA_ERROR_BAD_GLOBAL_PARAMETERS);
  ...
  //====================================================
  // Hcal  endcap modules
  theENDCAPEndSD = new SDAHcalEndCapScalable(Hcal_cell_dim_x,
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
void SHcalSc04::BarrelRegularModules(G4LogicalVolume* MotherLog)
{
...
//
//=======================================================================
void SHcalSc04::EndcapRingChambers(G4LogicalVolume* MotherLog, SDHcalEndCap* theSD, bool rings)
{
  // Chambers in the SHcalSc04::EndcapRings
  ...
  else Control::Abort("SHcalSc04: Invalid sensitive model parameter!",MOKKA_ERROR_BAD_GLOBAL_PARAMETERS);
...
// Build HCAL endcaps                                            ~
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
void SHcalSc04::EndcapsAhcal(G4LogicalVolume* MotherLog)
{
	...
	G4double HBU_half = (Hcal_endcap_module_width - 15.0)/2.; //360 mm HBU
	//To be scalable driver
	//calculate how many modules should be used for endcaps.
	//minmum unit is half HBU.

	const int MAX100_SCALABLE = 100; // make sure enough space for endcap modules

	int Scalable_Max_Endcap_Modules_Number = (int) (Hcal_module_radius / Hcal_endcap_module_width);
	//G4cout <<"Scalable_Max_Endcap_Modules_Number: " <<Scalable_Max_Endcap_Modules_Number<<G4endl;

	while ( sqrt( Hcal_endcap_module_width * Scalable_Max_Endcap_Modules_Number
		      * Hcal_endcap_module_width * Scalable_Max_Endcap_Modules_Number
		      + ( HBU_half * 2 + Hcal_endcap_services_module_width)
		      * ( HBU_half * 2 + Hcal_endcap_services_module_width) ) 
		> Hcal_module_radius ) 
	  {
	    Scalable_Max_Endcap_Modules_Number--;
	  }

	Hcal_endcap_module_number = Scalable_Max_Endcap_Modules_Number*2;


	//G4cout <<"Scalable_Max_Endcap_Modules_Number: " <<Scalable_Max_Endcap_Modules_Number<<G4endl;
	G4cout <<"Hcal_endcap_module_number: " <<Hcal_endcap_module_number
	       <<"  to be built"<<G4endl;
	theENDCAPEndSD->SetmaxScalableEndcapModuleNumber( Hcal_endcap_module_number );
	
	G4double theXOffset = - (Hcal_endcap_module_width + eps) * Hcal_endcap_module_number/2. - eps/2.; //scalable modules.
	
	G4double box_half_y[MAX100_SCALABLE];

	//calculater the maximum length for each module
	 
	for(G4int ii = 0; ii<Hcal_endcap_module_number; ii++)
	  {
	    G4double moduleXOffset = (ii < Hcal_endcap_module_number/2) 
	      ? (Hcal_endcap_module_width + eps)*ii + theXOffset 
	      : (Hcal_endcap_module_width + eps)*(ii+1) + theXOffset ;
	    G4cout << "moduleXOffset: " << moduleXOffset <<G4endl;
	
	    box_half_y[ii] = CalculateHalfYEndcapModule(moduleXOffset);
	  }

	// resize the central endcaps module, create the hole for beam pipe
	box_half_y[Scalable_Max_Endcap_Modules_Number-1] -= HBU_half; // the central hole
	box_half_y[Scalable_Max_Endcap_Modules_Number]   -= HBU_half; // the central hole


	G4double endcap_rmax = -1.0;

	for(int i = 0; i<(Hcal_endcap_module_number/2); i++)
	  {
	...
	G4double shift_x;
	G4double shift_y[MAX100_SCALABLE];
	...
	for (G4int endCapID = 1; endCapID <= Hcal_endcap_module_number; endCapID++)
	  {
	    ...
	    else Control::Abort("SHcalSc04: Invalid sensitive HCAL model !",MOKKA_ERROR_BAD_GLOBAL_PARAMETERS);
		...
		if (endCapID == (Hcal_endcap_module_number/2) || endCapID == (Hcal_endcap_module_number/2+1) ) //center with hole.
		  {
		    shift_y[endCapID-1] = box_half_y[endCapID-1] + HBU_half*2. + eps; 
		  }
		else {
		...
		if (endCapID >=2 && endCapID <=Hcal_endcap_module_number-1)
		  {
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
void SHcalSc04::EndcapChambersAhcal(G4LogicalVolume* MotherLog, 
				    SDAHcalEndCapScalable* theENDCAPEndSDi, 
				    G4int layer_id,
				    G4UserLimits* pULimits)
{
	...
	*/
	...
	shift_z = - Hcal_endcap_total_z/2 + Hcal_endcap_radiator_thickness + Hcal_chamber_thickness/2. + (layer_id-1)*layerThickness;
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
void SHcalSc04::EndcapsAhcalFrontEndElectronics(G4LogicalVolume* MotherLog, G4int layer_id)
{
	/*------------------------------------------------------------------------------------------
	 General variables      
	 -----------------------------------------------------------------------------------------*/
	...
	// Check the service electronic thickness.
	if( (Hcal_steel_cassette_thickness + HcalServices_outer_FR4_thickness + HcalServices_outer_Cu_thickness)
	    > Hcal_chamber_thickness )
	  {
	    G4cout <<" Possible due to the Mokka run time change,"<<G4endl;
	    G4cout <<" Hcal_chamber_thickness is not normal design,"<<G4endl;
	    G4cout <<" Please check the following parameters accordingly!" <<G4endl;
	    G4cout <<" Hcal_steel_cassette_thickness, HcalServices_outer_FR4_thickness and HcalServices_outer_Cu_thickness" <<G4endl;
	    Control::Abort("SHcalSc04: The services geometry may not be built properly !",MOKKA_ERROR_BAD_GLOBAL_PARAMETERS);
	  }
...
//~       Load event                                 ~
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
void SHcalSc04::LoadEvent(FILE* theSubDetectorEventHitsFileInput)
{
...
//~       Setup                                      ~
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
G4bool SHcalSc04::Setup(const CGAGeometryEnvironment &theGeometryEnvironment)
{
  ...
  if( Hcal_barrel_end_module_type != 1)
    Control::Abort("SHcalSc04: Sorry, but TESLA like end modules in barrel are not available with this driver.",MOKKA_ERROR_BAD_GLOBAL_PARAMETERS);
  ...
  layerThickness = Hcal_endcap_radiator_thickness +Hcal_chamber_thickness;
...
//~              EndcapRings                          ~
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
void SHcalSc04::EndcapRings(G4LogicalVolume* MotherLog)
{
  ...
  if (rOuter[0] <= rInner[0]) 
    G4Exception("SHcalSc04::EndcapRings() - not enough place for endcap rings (try a larger Hcal_nlayers number)!");
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
G4bool SHcalSc04::PostConstructAction(CGAGeometryEnvironment& )
{
  //
  ...
  G4cout << "SHcalSc04 information: Hcal_outer_radius = "
	 << Hcal_outer_radius
	 << "\n                       module thickness = "
	 << Hcal_total_dim_y
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
void SHcalSc04::CalculateFractTileSize(G4double x_length, 
...
/*void SHcalSc04::BarrelModuleGap(G4LogicalVolume *MotherLog) 
{
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
void SHcalSc04::BarrelHalfRegularChambersTrap(G4LogicalVolume *MotherLog, 
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
void SHcalSc04::BarrelChambersGap(G4LogicalVolume *MotherLog,
				  G4double chambers_y_off_correction)
{
...
// Calculate x-length of an HCAL barrel layer                  ~
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
void SHcalSc04::CalculateXLayer(G4int layer_id, G4int &logical_layer_id,
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
// Calculate y-length of an HCAL endcaps module active layer                  ~
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
G4double SHcalSc04::CalculateHalfYEndcapModule(G4double moduleXOffset)
{
  G4int    scalable_half_HBU_number = 0;
  G4double scalable_endcap_module_length = 0;
  while ( sqrt( moduleXOffset * moduleXOffset + scalable_endcap_module_length * scalable_endcap_module_length)
	  < Hcal_module_radius ) 
    {
      scalable_half_HBU_number++;
      scalable_endcap_module_length = (Hcal_endcap_module_width - 15.0)/2. * scalable_half_HBU_number + Hcal_endcap_services_module_width;
    }
 
  G4cout<<"scalable_endcap_module_length : (maximum) "<< (Hcal_endcap_module_width - 15.0)/2. * (scalable_half_HBU_number - 1) <<G4endl;
  return  (Hcal_endcap_module_width - 15.0)/2. * (scalable_half_HBU_number - 1) / 2.;
