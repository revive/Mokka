...
G4bool SFtd04::ContextualConstruct(const CGAGeometryEnvironment &env, G4LogicalVolume *worldLog)

{
  ...
  if(beamTubeClearance < (cables_thickness + (2.0*cable_shield_thickness) + 0.5 *mm) ) {
    G4cout << "SFtd04:Stop: Not enough space for inner support structure and cables: increase beamTubeClearance" << G4endl;
    exit(1);
  }
  ...
  do 
	...
	if( z_position <= SIT1_Half_Length_Z && outer_radius>=SIT1_Radius) {
	  G4cout << "SFtd04:Stop: Overlap between FTD1 and SIT1" << G4endl;
	  G4cout << "SFtd04:FTD1 Radius = " << outer_radius << "SIT1 Radius = " << SIT1_Radius << G4endl;
	  exit(1);
	}
	if( db->fetchDouble("z_position_ReltoTPCLength") != 0.0) {
	  G4cout << "SFtd04:Stop: The z position of FTD1 is not relative. The relative value will not be used. It should be set to 0.0 in the DB." << G4endl;
	  G4cout << "SFtd04:Stop: The z position of FTD1 is set by the distance between the centre of the sensitive layer and the max z of VTX layer 3." << G4endl;
	  exit(1);
	}
	...
	InnerCylinderInnerRadius1 = inner_radius - ( ( z_position - zEnd_IPOuterTube ) * beamTubeTangent ) - 0.5 * mm; 

	// check that there is no overlap with SIT1
	if( z_position <= SIT1_Half_Length_Z && outer_radius>=SIT1_Radius) {
	  G4cout << "SFtd04:Stop:Overlap between FTD2 and SIT1" << G4endl;
	  G4cout << "SFtd04:FTD2 Radius = " << outer_radius << "SIT1 Radius = " << SIT1_Radius << G4endl;
	  exit(1);
	}
	break;
	...
	if( z_position <= SIT2_Half_Length_Z && outer_radius>=SIT2_Radius) {
	  G4cout << "SFtd04:Stop:Overlap between FTD3 and SIT2" <<  G4endl;
	  G4cout << "SFtd04:FTD3 Radius = " << outer_radius << "SIT2 Radius = " << SIT2_Radius << G4endl;
	  exit(1);
	}
	break;
	...
	InnerCylinderInnerRadius2 = inner_radius - ( ( (Disks_Si_thickness) / 2.0 + Disks_Support_thickness ) * beamTubeTangent ) - 0.5 * mm; 

	if( db->fetchDouble("z_position_ReltoTPCLength") != 0.0) {
	  G4cout << "SFtd04:Stop: The z position of FTD7 is not relative. The relative value will not be used. It should be set to 0.0 in the DB." << G4endl;
	  G4cout << "SFtd04:Stop: The z position of FTD7 is set by the distance between the centre of the sensitive layer and the min z of the ECal Endcap." << G4endl;
	  exit(1);
	}
	break;

      default:
	G4cout << "SFtd04: Error disk number must be between 1-7: disk number = " << disk_number << G4endl;
	exit(1);
      }
      
      std::cout.precision(15);
      std::cout << "SFtd04: Disk:" << disk_number
		<< "\t z = " << z_position
		<< "\t inner_radius = " << inner_radius
		<< "\t outer_radius = " << outer_radius
		<< "\t Disks_Si_thickness = " << Disks_Si_thickness
		<< "\t beamtube radius = " << beamTubeRadius
		<< "\t cables_thickness = " << cables_thickness
		<< "\t 2 x cable_shield_thickness = " << 2 * cable_shield_thickness
