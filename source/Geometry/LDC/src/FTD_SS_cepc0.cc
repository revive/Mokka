...
G4bool FTD_SS_cepc0::ContextualConstruct(const CGAGeometryEnvironment &env, G4LogicalVolume *worldLog)
{
  ...
  do {
	...
	if( _z_position <= _glEnv.SIT1_Half_Length_Z && _outer_radius>=_glEnv.SIT1_Radius) 
	  {
            G4cout << "FTD_SS_cepc0:Stop: Overlap between FTD1 and SIT1" << G4endl;
            G4cout << "FTD_SS_cepc0:FTD1 Radius = " << _outer_radius << "SIT1 Radius = " << _glEnv.SIT1_Radius << G4endl;
            exit(1);
	  }
	if( db->fetchDouble("z_position_ReltoTPCLength") != 0.0) 
	  {
            G4cout << "FTD_SS_cepc0:Stop: The z position of FTD1 is not relative. The relative value will not be used. It should be set to 0.0 in the DB." << G4endl;
            G4cout << "FTD_SS_cepc0:Stop: The z position of FTD1 is set by the distance between the centre of the sensitive layer and the max z of VTX layer 3." << G4endl;
            exit(1);
	  }
	break;
	...
	if( _z_position <= _glEnv.SIT1_Half_Length_Z && _outer_radius>=_glEnv.SIT1_Radius) 
	  {
            G4cout << "FTD_SS_cepc0:Stop:Overlap between FTD2 and SIT1" << G4endl;
            G4cout << "FTD_SS_cepc0:FTD2 Radius = " << _outer_radius << "SIT1 Radius = " << _glEnv.SIT1_Radius << G4endl;
            exit(1);
	  }
	break;
	...
	if( _z_position <= _glEnv.SIT2_Half_Length_Z && _outer_radius>=_glEnv.SIT2_Radius) 
	  {
            G4cout << "FTD_SS_cepc0:Stop:Overlap between FTD3 and SIT2" <<  G4endl;
            G4cout << "FTD_SS_cepc0:FTD3 Radius = " << _outer_radius << "SIT2 Radius = " << _glEnv.SIT2_Radius << G4endl;
            exit(1);
	  }
	break;
	...
	if( db->fetchDouble("z_position_ReltoTPCLength") != 0.0) 
	  {
            G4cout << "FTD_SS_cepc0:Stop: The z position of FTD7 is not relative. The relative value will not be used. It should be set to 0.0 in the DB." << G4endl;
            G4cout << "FTD_SS_cepc0:Stop: The z position of FTD7 is set by the distance between the centre of the sensitive layer and the min z of the ECal Endcap." << G4endl;
            exit(1);
	  }
	break;
          
      default:
	G4cout << "FTD_SS_cepc0: Error disk number must be between 1-7: disk number = " << disk_number << G4endl;
	exit(1);
      }
    
    G4cout << "FTD_SS_cepc0: Disk:" << disk_number
	   << "\t z = " << _z_position
    ...
    // we will not construct the outer 2 layer.
    if (disk_number>5)
      continue;	
...
//                               build.
//
void FTD_SS_cepc0::DoAndPlaceDisk( std::map<std::string,G4double> valuesDict, G4LogicalVolume * mother )
{
...
void FTD_SS_cepc0::petalSupport( std::map<std::string,G4double> valuesDict, G4LogicalVolume * FTDPetalAirLogical )
{
...
void FTD_SS_cepc0::petalSensor( std::map<std::string,G4double> valuesDict, G4LogicalVolume * FTDPetalAirLogical )
{
...
G4double FTD_SS_cepc0::Getdy(const G4double & innerRadius )
{
...
G4double FTD_SS_cepc0::Getdx( const G4double & innerRadius )
{
...
std::vector<G4double> * FTD_SS_cepc0::GetPetalDimensions( const G4double& petal_cp_support_dy, const G4String & whereItgoes, const G4bool isSilicon )
{
  const G4double theta = _dbParCommon.petal_half_angle_support;
  ...
  if( whereItgoes == "UP" )
    ...
    {
      G4cout << "FTD_SS_cepc0: Internal Error: The function FTD_SS_cepc0::SemiPetalSolid is not well called, the " <<
...
//    //                   whereItgoes:         "UP" or "DOWN", where is placed
//    //                   isSilicon:           True of False, define if is the sensor or the holes
G4Trap * FTD_SS_cepc0::SemiPetalSolid( const G4double& petal_cp_support_dy, const G4String& whereItgoes, const G4bool isSilicon )
{
  // Get dimensions
...
void FTD_SS_cepc0::SetEnvironPar( const CGAGeometryEnvironment & env )
{
...
void FTD_SS_cepc0::SetdbParCommon( Database * db )
{
  ...
  if( _dbParCommon.beamTubeClearance < (_dbParCommon.cables_thickness + (2.0*_dbParCommon.cable_shield_thickness) + 0.5 *mm) ) 
    {
      G4cout << "FTD_SS_cepc0:Stop: Not enough space for inner support structure and cables: increase beamTubeClearance" << G4endl;
      exit(1);
    }
...
#ifdef DEBUG_VALUES
  G4cout << "FTD_SS_cepc0:SetdbParCommon:\n" 
	 << "beamTubeClearance = " << _dbParCommon.beamTubeClearance << " \n"   
...
void FTD_SS_cepc0::SetParDisk( Database * db)
{
...
#ifdef DEBUG_VALUES
  G4cout << "FTD_SS_cepc0:SetParDisk:\n" 
	 << "disk_number = " << _dbParDisk.disk_number << " \n"   
...
void FTD_SS_cepc0::RegisterSDs( Database * db )
{
...
void FTD_SS_cepc0::registerPV(const G4PhysicalVolumesPair & pvPair )
{
  if( pvPair.first != 0 )
    {
...
G4double FTD_SS_cepc0::GetdEdx( const G4Material * material )
{
