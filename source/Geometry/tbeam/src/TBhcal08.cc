/* Author Roman Poeschl DESY
   ...
   Updated for CERN 2010 test beam geometry by Jacopo Nardulli: CERN. 
*/
...
INSTANTIATE(TBhcal08)
/*================================================================================*/
/*                                                                                */
/*                                                                                */
/*                                                                                */
/*================================================================================*/
  TBhcal08::TBhcal08() : VSubDetectorDriver("TBhcal08","TBhcal"),
			 db(0),
			 _aGeometryEnvironment("","",NULL, NULL),
			 config_angle(0)  
{
  checkForOverlappingVolumes = false;
#ifdef TBHCAL08_DEBUG
...
/*                                                                                */
/*================================================================================*/
G4bool TBhcal08::ContextualConstruct(const CGAGeometryEnvironment &aGeometryEnvironment,
				     G4LogicalVolume *WorldLog)
{
  G4cout<<"\n ***************************************************************"<<G4endl;
  G4cout<<" *                                                             *"<<G4endl;
  G4cout<<" *    Build HCAL based on the TBhcal08 driver                  *"<<G4endl;
  G4cout<<" *                                                             *"<<G4endl;
  G4cout<<" ***************************************************************"<<G4endl;
...
/*                                                                                */
/*================================================================================*/
void TBhcal08::FetchMySQLVariables()
{
  ...
  */
  ...
  db->exec("select * from HCAL_cern_2010_virt;");
  db->getTuple();

  HCAL_n_layers                    = db->fetchInt("HCAL_n_layers");

  /*Angela Lucaci-Timoce: The scintillator plane is 90x90 mm^2*/
  HCAL_Layer_ncell_x               = 90*mm;//db->fetchInt("HCAL_Layer_ncell_x");
  HCAL_Layer_ncell_y               = 90*mm;//db->fetchInt("HCAL_Layer_ncell_y");
  ...
  /*Absorber stuff*/
  db->exec("select * from HCAL_cern_2010_absorber_virt;");
  db->getTuple();
  ...
  /*Angela Lucaci-Timoce: The 2 lines below are wrong: the values are set to 0 and 5, but it should be -5, 5
  ...
  */
  ...
  /* Materials*/
  db->exec("select * from HCAL_cern_2010_materials_virt;");
  db->getTuple();
  ...
  /* thicknesses*/
  db->exec("select * from HCAL_cern_2010_layerthickness_virt;");
  db->getTuple();

  poly_hthickness                       = db->fetchDouble("poly_hthickness")/2;
  tungsten_hthickness                   = db->fetchDouble("tungsten_hthickness")/2;
  /*Angela Lucaci-Timoce: This value is again wrong: should be 0.5 mm, NOT 1 mm
  steel_support_hthickness              = db->fetchDouble("steel_support_thickness")/2;
  */
  steel_support_hthickness = 0.5/2.;

  airgap_hthickness                     = db->fetchDouble("airgap_hthickness")/2;
...
void TBhcal08::BuildHcalElements()
{
  ...
  */
  ...
  for (G4int iLayer = 0; iLayer < HCAL_n_layers; ++iLayer)
	...
	+ cablefibre_mix_hthickness;                   /*cable-fibre mix*/
      ...
      AluminiumframeSolid[iLayer]   = new G4SubtractionSolid("AluminiumframeAll - AbsLayerSolid", AluminiumframeAll[iLayer], AbsLayerSolid[iLayer]);
      AluminiumframeLogical[iLayer] = new G4LogicalVolume(AluminiumframeSolid[iLayer], aluminium,
							  G4String("HcalAluminiumframeLogical") + G4String(stringForLayerNo.str()), 0, 0, 0);
...
/*                                                                                */
/*================================================================================*/
G4bool TBhcal08::PlaceHcalLayers(G4LogicalVolume *WorldLog)
{
...
/*                                                                                */
/*================================================================================*/
void TBhcal08::DefineHcalMaterials() {
  ...
  */
  ...
  G4cout<<"\n  -----------> TBhcal08 material properties <----------------"<<G4endl;
...
/*                                                                                */
/*================================================================================*/
void TBhcal08::PlaceHcalElementsIntoLayer() 
{
...
#ifdef TBHCAL08_DEBUG
  G4cout<<" \n\n TBhcal08::PlaceHcalElementsIntoLayer() "<<G4endl;
  for (G4int iLayer = 0; iLayer < HCAL_n_layers; ++iLayer)
    {
