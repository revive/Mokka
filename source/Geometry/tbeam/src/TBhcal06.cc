...
INSTANTIATE(TBhcal06)

TBhcal06::TBhcal06() : VSubDetectorDriver("TBhcal06","TBhcal"),
		       db(0),
...
//G4bool TBhcal06::construct(const G4String &aSubDetectorDBName,
//			   G4LogicalVolume *WorldLog)
G4bool TBhcal06::ContextualConstruct(const
    CGAGeometryEnvironment &aGeometryEnvironment,
    G4LogicalVolume *WorldLog)
{
  G4cout << "\nBuilding TBhcal06..." << G4endl;
  ...
  G4cout << "\nDone building TBhcal06" << G4endl;
  
  return cokay;
}

// fetch MySQL variables
void TBhcal06::FetchAll()
{
...
void TBhcal06::BuildElements()
{
  ...
  foil_3m = TBhcal06::Polystyrole;
  pcb = TBhcal06::PCB;
  cf_mix = TBhcal06::CF_MIX;

  cal_hx = (G4double) (ncell_xy[0] * grid_size*mm)/2.;
  cal_hy = (G4double) (ncell_xy[1] * grid_size*mm)/2.;
  cal_hz = (G4double) (n_layers*layer_hthickness + n_layer_term*steel_hthickness_term);
...
void TBhcal06::PlaceLayer(G4LogicalVolume *WorldLog, G4int nlay)
{

  G4cerr << "Building hcal layer <" << nlay << ">." << G4endl;
  G4double lay_x = x_begin;
  G4double lay_y = y_begin;
  G4double lay_z = z_begin + layer_hthickness*(1.0/cos(config_angle));

  if (nlay > 1) {

    lay_z += (nlay-1) * (layer_hthickness*(1.0/cos(config_angle))*2.0);
  ...
  */
  ...
  // put in rotation
  G4double fullHCALThickness = n_layers*2.0*layer_hthickness*(1.0/cos(config_angle)) + n_layer_term*2.0*steel_hthickness_term*(1.0/cos(config_angle));
  G4double zOfRotationAxis = fullHCALThickness/2.0 + z_begin;
  
  G4double dZRotationAxis = zOfRotationAxis - lay_z;
  G4double zDisplacement = dZRotationAxis*cos(rotationAngle); // offset due to misalignments <- HcalRotationAngle
  
  G4double offsetX = dZRotationAxis*sin(rotationAngle); // offset due to misalignments <- HcalRotationAngle
  G4double offsetZ = zOfRotationAxis - zDisplacement - lay_z; // offset due to misalignments <- HcalRotationAngle
  ...
  if (nlay == n_layers) {
    ...
    for (int nterm = 0; nterm < n_layer_term; nterm++) {
      ...
      lay_x = x_begin;
      lay_y = y_begin;
      lay_z = z_begin + layer_hthickness*(1.0/cos(config_angle));
	...
	lay_z += layer_hthickness*(1.0/cos(config_angle)) + steel_hthickness_term*(1.0/cos(config_angle));
	
	dZRotationAxis = zOfRotationAxis - lay_z;  
	zDisplacement = dZRotationAxis*cos(rotationAngle);
  
	offsetX = dZRotationAxis*sin(rotationAngle);
	offsetZ = zOfRotationAxis - zDisplacement - lay_z;
	...
	lay_z += 2.0*steel_hthickness_term*(1.0/cos(config_angle));
...
void TBhcal06::SetSD()
{
  ...
  //defautl values:
  //   G4int    Hcal_apply_Birks_law = 1;
  //   G4double Hcal_time_cut = 150.;
...
void TBhcal06::AddMaterial() {
 ...
 // Chlorine
  ...
  G4cout << "New Materials for TBhcal06 implementation" << G4endl;
...
G4bool TBhcal06::isValidSensitiveLayerPattern(G4String sensitiveLayerPattern) {
...
std::vector<G4int> TBhcal06::getSensitiveLayerPatternVector(G4String sensitiveLayerPattern) {
...
void TBhcal06::buildLayerGeometry1() {
...
void TBhcal06::buildLayerGeometry2() {
