// $Header: /home/flc/cvs/Mokka/source/Geometry/tbeam/src/TBSD_VCell04.cc,v 1.7 2009/02/02 11:44:11 angela Exp $
...
TBSD_VCell04::TBSD_VCell04(G4String sdname,
			   G4double gridDim,
			   G4int nCellX,
			   G4int nCellY,
			   G4int dLayer,
...
void TBSD_VCell04::Initialize(G4HCofThisEvent* HCE)
{
#ifdef TBSD_DEBUG
...
G4bool TBSD_VCell04::ProcessHits(G4Step* aStep, G4TouchableHistory*)
{
#ifdef TBSD_DEBUG
  G4cout << G4endl << "TBSD_VCell04::ProcessHits(): " << SDName << G4endl;
#endif
 ...
 /***********************************************************/
  ...
  /*Angela Lucaci: for the TailCatcher, we have to add 1,
   otherwise we get layer numbers from 0 to 15*/
  if (SDName == "catcherSD") n_lay = n_lay + 1;
  ...
  if (!FindHit(cellId, edep , PDG, globalTime, sp) && 
...
G4bool TBSD_VCell04::FindHit(G4int cellId, G4double edep, G4int PDG, G4double globalTime, float* sp )
{
  TBHitMapIterator iter;

  for (iter=hitMap->begin(); iter != hitMap->end(); iter++) {
    if ( ( (iter->first)-cellId) == 0) {
      (iter->second)->AddEdep( Control::primaryId, PDG, edep, globalTime/ns, sp ) ; 
      return true;
    }		       
  } 
  return false;
...
void TBSD_VCell04::SetCellID( G4float localPosX, G4float localPosY){
...
void TBSD_VCell04::SetNCellXY(G4int nx, G4int ny) 
{ 
  // set n cell
  ncell_xy[0] = nx; 
  ncell_xy[1] = ny; 
...
void TBSD_VCell04::SetGridDim(G4double g) 
{ 
  gridDim = g; 
}

void TBSD_VCell04::SetDepthToLayer(G4int d) 
{ 
  depthToLayer = d; 
}

void TBSD_VCell04::SetModuleID(G4int m)
{
  moduleID = m;
}

G4int* TBSD_VCell04::GetNCellXY() 
{ 
  // get n cell
  return ncell_xy; 
} 

G4double TBSD_VCell04::GetGridDim() 
{ 
  return gridDim; 
}

G4int TBSD_VCell04::GetDepthToLayer() 
{ 
  return depthToLayer; 
}

G4int TBSD_VCell04::GetApplyBirksLaw() 
{ 
  return applyBirksLaw; 
}

G4double TBSD_VCell04::GetZBeginDetector()
{
  return zBeginDetector;
}

G4double TBSD_VCell04::GetHcalTimeCut()
{
  return hcalTimeCut;
} 

void TBSD_VCell04::EndOfEvent(G4HCofThisEvent* HCE)
{
  TBHitMapIterator iter;
...
G4double TBSD_VCell04::GetBirksAttenuatedEnergy(const G4Step* aStep)
{
