...
// chambers in the 2006 CERN setup    #
//                                    #
//#####################################

#include "TBdchXY03_01.hh"
...
INSTANTIATE(TBdchXY03_01)

TBdchXY03_01::~TBdchXY03_01()
{}

G4bool TBdchXY03_01::ContextualConstruct(const CGAGeometryEnvironment &aGeometryEnvironment,
...
// ecal_front_face = aGeometryEnvironment.GetParameterAsDouble("ecal_begin_z");
 ...
 G4cout << "\nDone building TBdchXY03_01" << G4endl;
 return doDch;

}

G4bool TBdchXY03_01::DchConstruct(G4LogicalVolume *WorldLog, 
			     ...
			     G4int idch)
{
  ...
  // do build process
  G4bool cokay = BuildDch(x_place, y_place, z_place, idch);

  return cokay;
}

void TBdchXY03_01::FetchAll()
{
  ...
  ncell_xy[0] = db->fetchDouble("dch_dim_x");		// 108 mm
  ncell_xy[1] = db->fetchDouble("dch_dim_y");		// 108 mm
  assert(ncell_xy[0]>0 && ncell_xy[1]>0);

  z_place1 = db->fetchDouble("z_place1");		// 
  z_place2 = db->fetchDouble("z_place2");		// 
  z_place3 = db->fetchDouble("z_place3");		// 
...
// take into account configuration angle and translation
  ...
  gas_pressure = db->fetchDouble("gas_pressure");               // 1.5 mbar
...
void TBdchXY03_01::BuildElements()
{
...
G4bool TBdchXY03_01::BuildDch(G4double x_place, G4double y_place, G4double z_place, G4int idch)
{
...
void TBdchXY03_01::SetSD(G4int idch)
{
...
void TBdchXY03_01::SetDepthToLayer(G4int i) {
  depthToLayer = i;
  G4cout <<" DepthToLayer in Dch: " << depthToLayer << G4endl;
}

void TBdchXY03_01::Print()
{
  G4cout << "\nTBdchXY03_01 information: " << G4endl
