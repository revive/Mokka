// $Header: /home/flc/cvs/Mokka/source/Geometry/tbeam/src/TBhcal05.cc,v 1.8 2008/03/21 19:50:35 angela Exp $
...
INSTANTIATE(TBhcal05)

TBhcal05::TBhcal05() : VSubDetectorDriver("TBhcal05","TBhcal"),
		       db(0),
...
//G4bool TBhcal05::construct(const G4String &aSubDetectorDBName,
//			   G4LogicalVolume *WorldLog)
G4bool TBhcal05::ContextualConstruct(const
    CGAGeometryEnvironment &aGeometryEnvironment,
    G4LogicalVolume *WorldLog)
{
  G4cout << "\nBuilding TBhcal05..." << G4endl;
  ...
  G4cout << "\nDone building TBhcal05" << G4endl;
  
  return cokay;
}

// fetch MySQL variables
void TBhcal05::FetchAll()
{

  // config angle from environment object
  // At CERN 2006 the HCAL was not rotatable. Need to be changed for the 2007 runs.
...
void TBhcal05::BuildElements()
{
  ...
  foil_3m = TBhcal05::Polystyrole;
  pcb = TBhcal05::PCB;
  cf_mix = TBhcal05::CF_MIX;
  ...
  airGapThickness = steel_cassette_hthickness + foil_hthickness + poly_hthickness + foil_hthickness + pcb_hthickness + cablefibre_mix_hthickness 
    + steel_cassette_hthickness;
  ...
  AirGap_LayerLogical = new G4LogicalVolume(AirGap_LayerSolid,
					    air,
...
#ifdef NO_VIS_HCAL
  AirGap_LayerLogical->SetVisAttributes(G4VisAttributes::Invisible);
#endif
  if( displayMode < DM_FULL )  
    AirGap_LayerLogical->SetVisAttributes(G4VisAttributes::Invisible);
...
void TBhcal05::PlaceLayer(G4LogicalVolume *WorldLog, G4int nlay)
{
  ...
  //  G4double lay_z = z_begin + layer_hthickness;
  G4double lay_z = (z_begin + layer_hthickness)*cos(config_angle) - x_begin*sin(config_angle);
  G4double lay_x = (z_begin + layer_hthickness)*sin(config_angle) + x_begin*cos(config_angle);
  ...
  if (nlay > 1) {
    ...
    //lay_z += ((nlay-1) * (layer_hthickness*2));
    lay_z += ((nlay-1) * (layer_hthickness*2))*cos(config_angle);
    lay_x += ((nlay-1) * (layer_hthickness*2))*sin(config_angle);
  ...
  */
  ...
  // rotate all the hcal layers to be perpendicular to the beam, because the HCAL was not rotatable during the CERN measurements in the year 2006.
  // needs to be adapted if the HCAL is rotatable
  G4RotationMatrix rotateHCALLayer;
  rotateHCALLayer.rotateY(config_angle);

  G4ThreeVector translateHCALLayer(lay_x,lay_y,lay_z);
  G4Transform3D* transformHCALLayer = new G4Transform3D(rotateHCALLayer,translateHCALLayer);


  // place sensor casette or air gap
  if ( sensitiveLayerPatternVector.at(nlay-1) == 0 ){
    // place an air gap
    new G4PVPlacement(*transformHCALLayer,
		      WholeLayer2Logical,
  ...
  if (nlay == n_layers) {
    ...
    for (int nterm = 0; nterm < n_layer_term; nterm++) {
      ...
      if (nterm == 0 ) {
	lay_z += (layer_hthickness + steel_hthickness_term)*cos(config_angle);
	lay_x += (layer_hthickness + steel_hthickness_term)*sin(config_angle);
	// lay_y = lay_y;
      }
      if (nterm > 0 ) {
	lay_z += (2*steel_hthickness_term)*cos(config_angle);
	lay_x += (2*steel_hthickness_term)*sin(config_angle);
	// lay_y = lay_y;
      }
      ...
      G4cout << "y Position of Terminating Absorber: " << lay_y << G4endl;
      G4cout << "z Position of Terminating Absorber: " << lay_z << G4endl;
      */
      
      // rotate all the hcal layers to be perpendicular to the beam, because the HCAL was not rotatable during the CERN measurements in the year 2006.
      // needs to be adapted if the HCAL is rotatable
      G4RotationMatrix rotateTermLayer;
      rotateTermLayer.rotateY(config_angle);

      G4ThreeVector translateTermLayer(lay_x,lay_y,lay_z);
      G4Transform3D* transformTermLayer = new G4Transform3D(rotateTermLayer,translateTermLayer);

      std::stringstream sterm;
      sterm << nterm;
      
      new G4PVPlacement(*transformTermLayer,
			AbsLayerLogical_term,
...
void TBhcal05::Print()
{
  G4cout << "\nTBhcal05 parameters: " << G4endl
	 << "n_layers: " << n_layers << "  " << "n_layer_term: " << n_layer_term << G4endl
    	 << "layer_thickness: " << 2.0*layer_hthickness << "  " << "term_layer_thickness (absorber): " << steel_hthickness_term*2.0 << G4endl
         << "grid size: " << grid_size << G4endl
	 << "(x_begin,y_begin,z_begin): " << "(" << x_begin << "," << y_begin << "," << z_begin << ")" << G4endl
...
G4bool TBhcal05::isValidSensitiveLayerPattern(G4String sensitiveLayerPattern) {
...
std::vector<G4int> TBhcal05::getSensitiveLayerPatternVector(G4String sensitiveLayerPattern) {
...
void TBhcal05::buildLayerGeometry1() {
  
  // original geometry of the sensitive cassette
...
void TBhcal05::buildLayerGeometry2() {
  
  // geometry of the sensitive cassette where the order of scintillator and the PCB + cable fibre mix is swaped
  ...
  G4double pos_cfmix = pos_house1 + steel_cassette_hthickness + cablefibre_mix_hthickness; 
  G4cout << "Cable-Fibre mixture at: " << pos_cfmix << " mm" << G4endl;
  ...
  G4double pos_pcb = pos_cfmix + cablefibre_mix_hthickness + pcb_hthickness; 
  G4cout << "PCB at: " << pos_pcb << " mm" << G4endl;
  ...
  //Put first 3M foil Layer into the complete layer
  G4double pos_foil_1 = pos_pcb + pcb_hthickness + foil_hthickness; 
  G4cout << "3M Foil 1 at: " << pos_foil_1 << " mm" << G4endl;
  new G4PVPlacement(0,
		    G4ThreeVector(0,0,pos_foil_1),
		    FoilLayerLogical_1,
  ...
  //Put scintillator housing rear plate into the layer
  G4double pos_house2 = pos_foil_2 + foil_hthickness + steel_cassette_hthickness;
  G4cout << "Scintillator Housing Rearplate at: " << pos_house2 << " mm" << G4endl;
  new G4PVPlacement(0,
		    G4ThreeVector(0,0,pos_house2),
		    ScinHousLogical,
