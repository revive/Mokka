...
// chambers in the 2010 CERN setup    #
//#####################################

#include "TBdchXY06.hh"
#include "Control.hh"
#include "CGADefs.h"
...
INSTANTIATE(TBdchXY06)
  
  TBdchXY06::~TBdchXY06()
{}

G4bool TBdchXY06::ContextualConstruct(const CGAGeometryEnvironment &aGeometryEnvironment,
					 G4LogicalVolume *theWorld)
{
  ...
  G4cout << "\nDone building TBdchXY06" << G4endl;

  return doDch;
  
}

G4bool TBdchXY06::DchConstruct(G4LogicalVolume *WorldLog, 
				  G4double x_place, G4double y_place, G4double z_place, 
				  G4int idchamber)
{
  
  G4cout << " Building Drift Chamber " << idchamber << G4endl;
...
void TBdchXY06::FetchAll()
{
  config_angle = config_angle*deg;
  
  db->exec("select * from wch_cern_2010_virt;");
  db->getTuple();
  ...
  db->exec("select * from wch_cern_2010_layerthickness_virt;");
  db->getTuple();
  ...
  db->exec("select * from wch_cern_2010_materials_virt;");
  db->getTuple();
  //  Wire_chambers_CO2_density     = 1.98e-3*g/cm3;	
  // Wire_chambers_ArCO2_density  = 1.821e-3*g/cm3;
...
G4bool TBdchXY06::BuildDchY(G4double x_place, G4double y_place, G4double z_place, G4int idch)
{
...
G4bool TBdchXY06::BuildDchX(G4double x_place, G4double y_place, G4double z_place, G4int idch)
{                                     
  ...
  G4cout << " Back window Z position: " << wback_z << " mm " << G4endl;
...
void TBdchXY06::SetSD(G4int idch)
{
...
void TBdchXY06::SetDepthToLayer(G4int i) {
  depthToLayer = i;
  G4cout <<" DepthToLayer in Dch: " << depthToLayer << G4endl;
}

void TBdchXY06::Print()
{
  G4cout << "\nTBdchXY06 information: " << G4endl
