...
//  muon counters in the Aug06 CERN setup #
//                                        #
//#########################################

#include "TBmuoncount01.hh"
...
INSTANTIATE(TBmuoncount01)

TBmuoncount01::~TBmuoncount01()
{}

G4bool TBmuoncount01::ContextualConstruct(const CGAGeometryEnvironment &aGeometryEnvironment,
                  G4LogicalVolume *theWorld)
{
 ...
 G4bool doMC2 = SciConstruct (theWorld,ncell_xy[2],ncell_xy[3],x_place2,y_place2,z_place2,2); // 1000x1000 counter

 delete db;
 db = 0;
  
 G4bool doMC = false;
 if (doMC1 && doMC2) doMC = true;

 G4cout << "\nDone building TBmuoncount01" << G4endl;
 return doMC;

}

G4bool TBmuoncount01::SciConstruct(G4LogicalVolume *WorldLog, G4double xdim, G4double ydim,
...
void TBmuoncount01::FetchAll()
{
  ...
  ncell_xy[0] = db->fetchDouble("mc_dim_x1");		// 1000 mm
  ncell_xy[1] = db->fetchDouble("mc_dim_y1");		// 1000 mm
  assert(ncell_xy[0]>0 && ncell_xy[1]>0);
  ncell_xy[2] = db->fetchDouble("mc_dim_x2");           // 1000 mm
  ncell_xy[3] = db->fetchDouble("mc_dim_y2");		// 1000 mm
  ...
  x_place1 = translateX + (z_place1)*sin(config_angle);
  y_place1 = translateY;
  z_place1 = (z_place1)*cos(config_angle);

  x_place2 = translateX + (z_place2)*sin(config_angle);
  y_place2 = translateY;
  z_place2 = (z_place2)*cos(config_angle);

  sci_hthickness = db->fetchDouble("mc_thickness");            // 7.5 mm
  sci_window = db->fetchDouble("steel_thickness");             // 1 mm

  Print();
}

void TBmuoncount01::BuildElements(G4double xdim, G4double ydim)
{
...
G4bool TBmuoncount01::BuildSci(G4double x_place, G4double y_place, G4double z_place,
			   G4int idsc)
{
...
void TBmuoncount01::SetSD(G4int idch)
{
...
void TBmuoncount01::SetDepthToLayer(G4int i) {
  depthToLayer = i;
  G4cout <<" DepthToLayer in Muon Counter: " << depthToLayer << G4endl;
}

void TBmuoncount01::Print()
{
  G4cout << "\nTBmuoncount01 information: " << G4endl
