...
//*******************************************************
//
// $Id: HcalRPC.cc,v 1.5 2006/01/26 16:11:16 adrian Exp $
// $Name: mokka-07-00 $
//
// HcalRPC.cc
//
...
G4bool 
HcalRPC::construct(const G4String &aSubDetectorName,
		  G4LogicalVolume *WorldLog)
{
  ...
  db->exec("select cell_dim_x, cell_dim_z, radius, chamber_tickness, divide from barrel_module,hcal;");
  db->getTuple();
  ...
  G4double radius = db->fetchDouble("radius");
  G4String divideFlag = db->fetchString("divide");
  G4bool divide = false;
  if(divideFlag == "false")
	  ;
  else if(divideFlag == "true")
	  divide = true;
  else Control::Abort("Invalid divide flag in the dababase!",
...
//~              Barrel
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
void HcalRPC::Barrel(G4LogicalVolume* MotherLog)
{
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
void HcalRPC::BarrelRegularModules(G4LogicalVolume* MotherLog)
{
  // Regular modules
...
//   // BarrelStandardModule placements
  db->exec("select stave_id,module_id,module_type,module_theta_offset, stave_phi_offset,inner_radius,module_z_offset from barrel,barrel_stave, barrel_module, barrel_modules where module_type = 1;"); //  un module: AND stave_id=1 AND module_id = 2
  db->getTuple();
  G4double Y;
  Y = 0;
  ...
  do {
    ...
    thetarot = db->fetchDouble("module_theta_offset")*pi/180;
    G4RotationMatrix *rot=new G4RotationMatrix();
    rot->rotateX(thetarot); // on couche le module.
    rot->rotateY(phirot);
    G4ThreeVector pos = G4ThreeVector(0.0,db->fetchDouble("module_z_offset"),Y);
    pos = (*rot) * pos;
    /*
   ...
   */
    ...
    theBarrilRegSD->SetModuleRotationMatrix(pi*0.5-thetarot);
    theBarrilRegSD->SetStaveRotationMatrix(db->fetchInt("stave_id"),phirot);
    theBarrilRegSD->
      SetModuleZOffset(db->fetchInt("module_id"),pos(2));
//		       db->fetchDouble("module_z_offset"));
  } while(db->getTuple()!=NULL);
}

void HcalRPC::BarrelRegularChambers(G4LogicalVolume* MotherLog,G4double chambers_y_off_correction)
{
  ...
  while(db->getTuple()!=NULL){
      ...
      if (SensitiveModel == "RPC1")
	{
      ...
      else Control::Abort("Invalid sensitive model in the dababase!",
		MOKKA_ERROR_BAD_DATABASE_PARAMETERS);
  ...
  while(db->getTuple()!=NULL){    
    ...
    theBarrilRegSD->
      AddLayer(layer_id, pos(0), pos(1), pos(2));
  }
}
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
/*
void HcalRPC::BarrelEndModules(G4LogicalVolume* MotherLog)
{
  // End modules
  // Get parameters
...
void HcalRPC::BarrelEndChambers(G4LogicalVolume* MotherLog, 
			     G4double chambers_y_off_correction)
{
  ...
  G4UserLimits* pULimits=
   ...
   while(db->getTuple()!=NULL)
	 ...
	 else Control::Abort("Invalid sensitive model dans la dababase!");
       ChamberLog[db->fetchInt("layer_id")]->SetVisAttributes(VisAtt);
     }
...
//~              Endcaps                 ~
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
void HcalRPC::Endcaps(G4LogicalVolume* MotherLog)
{
...
void HcalRPC::EndcapChambers(G4LogicalVolume* MotherLog)
{
  // Chambers in the HcalRPC::Endcaps
...
*/
G4LogicalVolume * 
HcalRPC::BuildRPC1Box(G4Box* ChamberSolid, 
		     SD02* theSD, 
		     G4int layer_id,
		     G4UserLimits* pULimits)
{
  ...
  Control::Abort("HcalRPC::BuildRPC1Box: invalid ChamberSolidEnvelope",MOKKA_OTHER_ERRORS);
  return NULL;
}
/*
G4LogicalVolume * 
HcalRPC::BuildRPC1Polyhedra(G4Polyhedra* ChamberSolid, 
			   SD* theSD,
			   G4double phiStart,
			   G4double phiTotal,
			   G4int numSide,
			   ...
			   G4UserLimits* pULimits)
{
  ...
  Control::Abort("HcalRPC::BuildRPC1Polyhedra: invalid ChamberSolidEnvelope");
  return NULL;  
}
*/
void 
HcalRPC::LoadEvent(FILE* theSubDetectorEventHitsFileInput)
{
