...
//*******************************************************
//
// $Id: Proto04.cc,v 1.4 2009/02/11 13:32:46 musat Exp $
// $Name: mokka-07-00 $
//
//
// Proto04.cc
...
G4bool Proto04::ContextualConstruct(
...
G4bool Proto04::construct(const G4String &aSubDetectorName,
			  G4LogicalVolume *WorldLog)

{
  G4cout << "\nBuilding Proto release 04" << G4endl;
  ...
  if (HalfEcalX <= HalfDeadWX * 3) {
	G4cout << "======= ASSERT WILL CRASH :\n"
		...
		<< HalfEcalX * 2 << G4endl;
        Control::Abort("Proto04::construct: Assertion failed (HalfEcalX <= HalfDeadWX * 3)",MOKKA_OTHER_ERRORS);
  }
#endif
  ...
  if (HalfDeadWZ  > HalfEcalZ) {
	G4cout << "======= ASSERT WILL CRASH :\n"
		...
		<< G4endl;
        Control::Abort("Proto04::construct: Assertion failed (HalfDeadWZ  > HalfEcalZ)",MOKKA_OTHER_ERRORS);
  }
#endif
...
//  }
  ...
  if(db->getTuple())
    ...
    {
      Control::Log("proto04 Warning: old database release without (x_center,y_center,z_center) values, assuming (0.,0., 2*HalfEcalZ).");
  ...
  //--------------
  char buff[80];
  sprintf(buff,"proto04: proto size is (%f,%f,%f) mm",2*HalfEcalX,
	  2*HalfEcalZ,2*HalfEcalY);
  Control::Log(buff);
  sprintf(buff,"proto04: placing prototype at (%f,%f,%f) mm",
...
void Proto04::BuildStructures(void) {
...
for (unsigned int i_group=0;i_group<PlateGroups.size();i_group++) {
  ...
  EnvStructureLogical->SetVisAttributes(VisAttAir);
    ...
    if (HalfEnvStructX < HalfEcalX) {
		...
		<< " in the X direction\n" << G4endl;
        Control::Abort("Proto04::BuildStructures: Assertion failed (HalfEnvStructX < HalfEcalX)",MOKKA_OTHER_ERRORS);
    }
#endif
...
void Proto04::DefineMaterial(void) {
...
void Proto04::BuildElements() 
{
  ...
  if ((HalfWSlabX > HalfAlveolusX) || (HalfWSlabZ > (HalfAlveolusZ - al_thickness))) {
	G4cout << "======= ASSERT WILL CRASH :\n"
		...
		<< " in the X-Y plane\n" << G4endl;
        Control::Abort("Proto04::BuildElements: Assertion failed ((HalfWSlabX > HalfAlveolusX) || (HalfWSlabZ > (HalfAlveolusZ - al_thickness)))",MOKKA_OTHER_ERRORS);
  }
#endif
...
void Proto04::BuildDeadPlate(WLAYERS* aPlate)
{
  //-----------------------------------
...
void Proto04::CalculateSlabShifts(void) {
...
void Proto04::BuildAlveola(WLAYERS* aPlate) {
...
void Proto04::FillAlveola(WLAYERS* aPlate, G4LogicalVolume * CFCenterLogical,
				G4LogicalVolume * CFRightLogical) {
  ...
  if (check < 0.) {
	G4cout << "======= ASSERT WILL CRASH :\n"
		...
		<< G4endl;
        Control::Abort("Proto04::FillAlveola: Assertion failed (check < 0.) (ID 1)",MOKKA_OTHER_ERRORS);
  }
#endif
  ...
  if(check < 0.) {
	G4cout << "======= ASSERT WILL CRASH :\n"
		...
		<< G4endl;
        Control::Abort("Proto04::FillAlveola: Assertion failed (check < 0.) (ID 2)",MOKKA_OTHER_ERRORS);
  }
#endif
...
void Proto04::PlaceWafers(G4LogicalVolume * motherTop, 
...
void  
Proto04::BeginOfEventAction(const G4Event*)
{
  Dumped=false;
}

void 
Proto04::EndOfEventAction(const G4Event* evt)
{
  if(!Dumped) 
