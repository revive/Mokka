...
//*******************************************************
//
// $Id: Proto02.cc,v 1.3 2005/04/08 14:37:15 musat Exp $
// $Name: mokka-07-00 $
//
//
// Proto02.cc
...
G4bool Proto02::construct(const G4String &aSubDetectorName,
			  G4LogicalVolume *WorldLog)

{
  G4cout << "\nBuilding Proto release 02" << G4endl;
  PlateGroups.clear();
  Plates.clear();
  ...
  db->exec("select x_center,y_center,z_center, start_layer_number from proto;");
  if(db->getTuple())
    {
  ...
  for(G4int i_tower=0;i_tower<n_towers;i_tower++) 
      ...
      for(i_plate=0;i_plate<total_W_plates;i_plate++)
	{
	  ...
	  if(i_plate%2==0) // Dead plates
		...
		+ fiber_thickness  / 2. ;
	      DispZ=DispZTower;
	      CopyNumber= i_tower*100 + i_plate + start_layer_number;
	      new G4PVPlacement(0,
...
void Proto02::BuildElements() 
{
  ...
  while (db->getTuple())
	...
	push_back(new WLAYERS(n_layers,
      ...
      G4cout << "plate_multiplicity = " << db->fetchInt("plate_multiplicity")
	     << G4endl;
  ...
  HalfSuppX = 
    ...
    db->fetchInt("multiplicity_waffers")*HalfWafferX +
  ...
  //-------------------------------------
  HalfAlveolusX =
   HalfSuppX * db->fetchInt("n_elements_x")/2. + 
  ...
  G4String theSDName = G4String("ProtoSD");
  if(theProtoSD==0)
    {
      theProtoSD = new
...
void Proto02::BuildDeadPlate(WLAYERS* aPlate)
{
  ...
  //
  G4VisAttributes * VisAttWBlock = new G4VisAttributes(G4Colour(0,0,1.));
  ...
  G4LogicalVolume* LastBk = 
    new G4LogicalVolume(DeadWSolid,
			...
			0, 
			0);
  G4VisAttributes * VisAttLastWBlock = new G4VisAttributes(G4Colour(0.0,0.,1.0));
  ...
  G4double BlockMultiplicityOffset = 0;
  db->exec("select * from proto;");
  db->getTuple();
  
  G4int multiplicity_waffers = db->fetchInt("multiplicity_waffers");
  if(multiplicity_waffers==2)
    BlockMultiplicityOffset =  - HalfLaneX;
  
  for (G4int iWaffer = 0; iWaffer < multiplicity_waffers; iWaffer++)
    {
	...
	{    
	      ...
	      for(iBlock=0; iBlock < n_X_blocks_std; iBlock++)
		{
		  ...
		  G4ThreeVector Pos =
		    G4ThreeVector(firstOffX + iBlock * 2*HalfBlockZ,
				  ...
				  firstOffZ + ilane * 2*HalfLaneX + BlockMultiplicityOffset);
      ...
      BlockMultiplicityOffset+= 2*HalfLaneX;
    }
...
void Proto02::BuildAlveolus(WLAYERS* aPlate)
{
...
//     db->fetchDouble("inter_waffer_gap")/2;
  ...
  XDisp= ((HalfSuppX+db->fetchDouble("inter_waffer_gap")/2) *
    (db->fetchInt("n_elements_x")-1))/2.;
...
//      XInterElementDisp=XInterElementDisp/2.;
...
// 				db->fetchDouble("inter_waffer_gap"));
	  //}
      XDisp-=(HalfSuppX+db->fetchDouble("inter_element_gap")/2);
    }
...
void  
Proto02::BeginOfEventAction(const G4Event*)
{
  Dumped=false;
}

void 
Proto02::EndOfEventAction(const G4Event* evt)
{
  if(!Dumped) 
