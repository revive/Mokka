...
//*******************************************************
//
// $Id: Proto04_02.cc,v 1.3 2009/02/11 13:32:46 musat Exp $
// $Name: mokka-07-00 $
//
//
// Proto04_02.cc
...
INSTANTIATE(Proto04_02)

G4bool Proto04_02::ContextualConstruct(
	...
	config_angle = aGeometryEnvironment.GetParameterAsDouble("EcalRotationAngle");
...
G4bool Proto04_02::construct(const G4String &aSubDetectorName,
			  G4LogicalVolume *WorldLog)

{
  G4cout << "\nBuilding Proto release 04_02" << G4endl;
  ...
  //read Y Si Layer shifts
  db->exec("select layer_shift from si_layers where position ='center' order by id;");
  while (db->getTuple()) {
      G4double layer_shift = db->fetchDouble("layer_shift");
      if(fabs(layer_shift) > 0.2) {
	Control::Abort("Proto04_02: YShift in center tower greater than 0.2 mm!",MOKKA_ERROR_BAD_DATABASE_PARAMETERS);
      }
      centerYLayerShifts.push_back(layer_shift);
  }

  db->exec("select layer_shift from si_layers where position ='right' order by id;");
  while (db->getTuple()) {
      G4double layer_shift = db->fetchDouble("layer_shift");
      if(fabs(layer_shift) > 0.2) {
	Control::Abort("Proto04_02: YShift in right tower greater than 0.2 mm!",MOKKA_ERROR_BAD_DATABASE_PARAMETERS);
      }
      rightYLayerShifts.push_back(layer_shift);
  }

#ifdef MOKKA_DEBUG
  if(centerYLayerShifts.size() != 30) 
	Control::Abort("table si_layer doesn't contain exactly 30 layer shifts for the center tower",MOKKA_ERROR_BAD_DATABASE_PARAMETERS);

  if(rightYLayerShifts.size() != 30)
	Control::Abort("table si_layer doesn't contain exactly 30 layer shifts for the right tower",MOKKA_ERROR_BAD_DATABASE_PARAMETERS);
  ...
  if (HalfEcalX <= HalfDeadWX * 3) {
	G4cout << "======= ASSERT WILL CRASH :\n"
		...
		<< HalfEcalX * 2 << G4endl;
        Control::Abort("Proto04_02::construct: Assertion failed (HalfEcalX <= HalfDeadWX * 3)",MOKKA_OTHER_ERRORS);
  }
#endif
  ...
  if (HalfDeadWZ  > HalfEcalZ) {
	G4cout << "======= ASSERT WILL CRASH :\n"
		...
		<< G4endl;
        Control::Abort("Proto04_02::construct: Assertion failed (HalfDeadWZ  > HalfEcalZ)",MOKKA_OTHER_ERRORS);
  }
#endif
  ...
  G4String theCellSDName = G4String("ProtoSD03_01");
  G4String theGRSDName = G4String("ProtoSD03_01GuardRing");

  if(theCellProtoSD==0)
    {
      theCellProtoSD = new
	ProtoSD03_01(cell_dim_x, cell_dim_z, n_cell_x, n_cell_z, 
		...
		lateralWaferGap, endcap_x, 
		centerYLayerShifts,rightYLayerShifts,
		useID1);
...
void Proto04_02::BuildStructures(void) {
...
for (unsigned int i_group=0;i_group<PlateGroups.size();i_group++) {
  ...
  EnvStructureLogical->SetVisAttributes(VisAttAir);
    ...
    if (HalfEnvStructX < HalfEcalX) {
		...
		<< " in the X direction\n" << G4endl;
        Control::Abort("Proto04_02::BuildStructures: Assertion failed (HalfEnvStructX < HalfEcalX)",MOKKA_OTHER_ERRORS);
    }
#endif
...
void Proto04_02::DefineMaterial(void) {
...
Proto04_02::~Proto04_02()
{
//   if(theProtoSD!=0) delete theProtoSD;
}

void Proto04_02::BuildElements() 
{
  //---------------------------------
  ...
  if ((HalfWSlabX > HalfAlveolusX) || (HalfWSlabZ > (HalfAlveolusZ - al_thickness))) {
	G4cout << "======= ASSERT WILL CRASH :\n"
		...
		<< " in the X-Y plane\n" << G4endl;
        Control::Abort("Proto04_02::BuildElements: Assertion failed ((HalfWSlabX > HalfAlveolusX) || (HalfWSlabZ > (HalfAlveolusZ - al_thickness)))",MOKKA_OTHER_ERRORS);
  }
#endif
...
void Proto04_02::BuildDeadPlate(WLAYERS* aPlate)
{
  //-----------------------------------
...
void Proto04_02::CalculateSlabShifts(void) {
...
void Proto04_02::BuildAlveola(WLAYERS* aPlate, unsigned int iPlate) {
  ...
  FillAlveola(aPlate, CFCenterLogical, CFRightLogical, iPlate);
...
void Proto04_02::FillAlveola(WLAYERS* aPlate, G4LogicalVolume * CFCenterLogical,
				G4LogicalVolume * CFRightLogical,
				unsigned int iPlate) {
  ...
  if (check < 0.) {
	G4cout << "======= ASSERT WILL CRASH :\n"
		...
		<< G4endl;
        Control::Abort("Proto04_02::FillAlveola: Assertion failed (check < 0.) (ID 1)",MOKKA_OTHER_ERRORS);
  }
#endif
  ...
  if(check < 0.) {
	G4cout << "======= ASSERT WILL CRASH :\n"
		...
		<< G4endl;
        Control::Abort("Proto04_02::FillAlveola: Assertion failed (check < 0.) (ID 2)",MOKKA_OTHER_ERRORS);
  }
#endif
  ...
  PlaceWafers(AirCenterTopLogical, AirCenterBottomLogical, SiO2WafferLogical,
	  ...
	  aPlate->center_waffer_shift, VerticalDisp, 0, 0,0,0);

  PlaceWafers(AirRightTopLogical, AirRightBottomLogical, SiO2WafferLogical,
	  SiO2WafferLogical, siLayerCenterX + xDisp,
	  aPlate->right_waffer_shift, VerticalDisp, 0, 0,0,0);
...
void Proto04_02::PlaceWafers(G4LogicalVolume * motherTop, 
		...
		G4int delta_j, G4int i, unsigned int iPlate, G4int iCorR) {  //center delta_i=0; right delta_i=n_waffers_z
  ...
  G4double zDisp = (n_waffers_z-1)*
	(HalfWafferZ+inter_waffer_gap/2.);
  G4double topYShift=0., bottomYShift=0.;

  switch(iCorR) {
	case 1: //center tower
		topYShift = centerYLayerShifts[2*iPlate];
		bottomYShift = centerYLayerShifts[2*iPlate+1];
		break;
	case 2: //right tower
		topYShift = rightYLayerShifts[2*iPlate];
		bottomYShift = rightYLayerShifts[2*iPlate+1];
		break;
  }; 
  ...
  for(G4int j=0;j<n_waffers_z;j++) {
	...
	new MyPlacement(0, G4ThreeVector(xDisp, -yDisp, zDisp+topYShift),
				    wafferToPlace,
				    WafferName,
				    motherTop,
				    false,-CopyNumber);
	new MyPlacement(0,G4ThreeVector(xDisp + bottomShift,
				    yDisp, zDisp+bottomYShift),
				    wafferToPlace,
				    WafferName,
				    motherBottom,
				    false,CopyNumber);
...
void  
Proto04_02::BeginOfEventAction(const G4Event*)
{
  Dumped=false;
}

void 
Proto04_02::EndOfEventAction(const G4Event* evt)
{
  if(!Dumped) 
